...
import { OPERATOR, CORPORATE, CONTACT, CONTACT2, APEX, APEX_AGENT, SURFACES_VERSION, ROBOTS, SECURITY, ICONS, esc, int, today, dl, num, table, form, headers, html, text, json, sitemap, page } from './chrome.js';
import { PRODUCTS, ORDER, nodesTable, totals } from './products.js';
import { heroRecord, homeBody, PAGES, readStatus, statusBody, statusJson, probe } from './site.js';
...
function nodeBody(host, cc, live) {
  ...
  return `<p class="lead">${esc(fill(d.reason, v))}</p><h2>1 Scope</h2>${s1}<h2>2 Access</h2>${s2}<h2>3 Residency</h2>${s3}<h2>4 Provenance</h2>${s4}<h2>5 Known gaps</h2>${s5}<h2>6 Market particulars</h2>${s6}<h2>7 Estate</h2>${s7}<h2>8 Machine kit</h2>${s8}<h2>9 Contact</h2>${s9}`;
}
...
export default {
  ...
  async fetch(request, env) {
    ...
    if (c.kind === 'site') {
      ...
      if (p === '/') { const rec = await heroRecord(); return html(page({ host, title: 'Allooloo Technologies Corp. — Agentic Capital Markets Global Desk', desc: `Allooloo builds the Capital Markets Knowledge Graph and serves Capital Markets Records to AI agents over MCP and A2A: ${t.records} issuer records, ${t.events} dated disclosure events, ${t.nodes} doors live, in-country. Licence only, no services.`, h1: 'Allooloo Technologies Corp. — Agentic Capital Markets Global Desk', state: 'live', asOf: t.last || today(), body: homeBody(ctx, host, rec), jsonld: [{ '@context': 'https://schema.org', '@type': 'Organization', name: OPERATOR, url: CORPORATE, logo: `${CORPORATE}/icon-512.png`, address: { '@type': 'PostalAddress', addressLocality: 'Vancouver', addressRegion: 'BC', addressCountry: 'CA' }, contactPoint: { '@type': 'ContactPoint', email: CONTACT, contactType: 'developers' } }] }), meta); }
