...
<h2>6 Where the record lives</h2><p>Records are stored and served in the issuer's jurisdiction: Canada Central Azure (Toronto, Canada), East US Azure (Virginia, United States), UK South Azure (London, United Kingdom), France Central Azure (Paris, France), West Europe Azure (Amsterdam, Netherlands), Switzerland North Azure (Zurich, Switzerland), Germany West Central Azure (Frankfurt, Germany), Australia East Azure (Sydney, Australia), Southeast Asia Azure (Singapore), Japan East Azure (Tokyo, Japan), Korea Central Azure (Seoul, South Korea). Hong Kong is a beacon; a local partner is wanted. The apex router holds an index and forwards. No record body leaves its jurisdiction.</p>
<h2>7 Contact</h2><p>Tell us what you trade.</p>${form(host)}<p class="muted">The agents that built this read their own mail: ${CONTACT2}. ${OPERATOR} · Vancouver, Canada.</p>`;
}
