...
const TOOLS = [
  ...
  { name: 'get_record', title: 'Get Capital Markets Record', annotations: { title: 'Get Capital Markets Record', ...ANN }, description: 'Return the full Capital Markets Record for an issuer, with per-field source, reader and state; optionally a prior version.',
    inputSchema: { type: 'object', properties: { identifier: { type: 'string', description: 'Ticker, ISIN, LEI, or a CMR key such as ca-cm-kg/TSX/SHOP, uk-cm-kg/LSE/SHEL, au-cm-kg/ASX/BHP, sg-cm-kg/SGX/D05, ch-cm-kg/SIX/NESN, de-cm-kg/XETRA/SAP, fr-cm-kg/XPAR/FR0000120271, nl-cm-kg/XAMS/NL0000009165, jp-cm-kg/TSE-PRIME/7203 or kr-cm-kg/KOSPI/005930.' }, version: { type: 'integer', description: 'Prior version number; omitted = current.' } }, required: ['identifier'] } },
...
const HOST_NODE = { 'mcp.ca-cm-kg.ai': 'ca-cm-kg', 'mcp.uk-cm-kg.ai': 'uk-cm-kg', 'mcp.au-cm-kg.ai': 'au-cm-kg', 'mcp.sg-cm-kg.ai': 'sg-cm-kg', 'mcp.ch-cm-kg.ai': 'ch-cm-kg', 'mcp.de-cm-kg.ai': 'de-cm-kg', 'mcp.fr-cm-kg.ai': 'fr-cm-kg', 'mcp.nl-cm-kg.ai': 'nl-cm-kg', 'mcp.jp-cm-kg.ai': 'jp-cm-kg', 'mcp.kr-cm-kg.ai': 'kr-cm-kg' };
...
const NODE_COPY = {
  ...
  'fr-cm-kg': { title: 'Capital Markets Knowledge Graph — France node door', scope: 'Scope: Euronext Paris, Euronext Growth Paris and Euronext Access Paris equity lines (ESMA FIRDS roster); records keyed by ISIN.', example: 'XPAR/FR0000120271', exchanges: 'XPAR, ALXP, XMLI', ids: 'FR0000120271, XPAR:FR0000120271, EPA:FR0000120271, TotalEnergies SE' },
  'nl-cm-kg': { title: 'Capital Markets Knowledge Graph — Netherlands node door', scope: 'Scope: Euronext Amsterdam equity lines (ESMA FIRDS roster); records keyed by ISIN.', example: 'XAMS/NL0000009165', exchanges: 'XAMS, ALXA', ids: 'NL0000009165, XAMS:NL0000009165, AMS:NL0000009165, Heineken N.V.' },
  'jp-cm-kg': { title: 'Capital Markets Knowledge Graph — Japan node door', scope: 'Scope: Tokyo Stock Exchange Prime, Standard, Growth and PRO Market lines and listed REITs / funds (JPX roster, EDINET filer master); Japanese names as published.', example: 'TSE-PRIME/7203', exchanges: 'TSE-PRIME, TSE-STANDARD, TSE-GROWTH, TSE-PRO, TSE-REIT', ids: '7203, TSE:7203, 7203.T, TYO:6758, Toyota Motor Corporation' },
  'kr-cm-kg': { title: 'Capital Markets Knowledge Graph — South Korea node door', scope: 'Scope: KOSPI, KOSDAQ and KONEX companies (KIND roster, DART filer master); Korean names as published.', example: 'KOSPI/005930', exchanges: 'KOSPI, KOSDAQ, KONEX', ids: '005930, KOSPI:005930, 005930.KS, KQ:035720, Samsung Electronics' },
};
function human(host, node) {
  const c = NODE_COPY[node] || { title: 'Capital Markets Knowledge Graph — global door', scope: `Routes by identifier to the node that holds the name. Live today: ${liveNodes().join(', ')}.`, example: 'uk-cm-kg/LSE/SHEL', exchanges: 'per node', ids: 'SHOP, SHEL, SHEL.L, AIM:4BB, BHP.AX, D05.SI, NESN.SW, SAP.DE, FR0000120271, NL0000009165, 7203.T, 005930.KS' };
...
GET paths: / (this door), /facts.json, /llms.txt, /nodes.json, /record/<EXCHANGE>/<TICKER> (node hosts) or /record/<node>/<EXCHANGE>/<TICKER> (any host), /events/… likewise with ?since=YYYY-MM-DD&cursor=0&limit=50. Exchanges: ${c ? c.exchanges : 'TSX, TSXV, CSE, CBOE-CANADA (ca-cm-kg); LSE, AIM, AQSE (uk-cm-kg); ASX, NSX (au-cm-kg); SGX, SGX-CATALIST (sg-cm-kg); SIX, BX (ch-cm-kg); XETRA, EUREX (de-cm-kg); XPAR, ALXP, XMLI (fr-cm-kg); XAMS, ALXA (nl-cm-kg); TSE-PRIME, TSE-STANDARD, TSE-GROWTH, TSE-PRO, TSE-REIT (jp-cm-kg); KOSPI, KOSDAQ, KONEX (kr-cm-kg)'}.
`;
}
export default {
