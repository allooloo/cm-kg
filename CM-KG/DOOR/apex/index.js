...
export default {
  async fetch(request, env) {
    ...
    if (['/favicon.ico', '/favicon.svg', '/apple-touch-icon.png', '/icon-192.png', '/icon-512.png', '/site.webmanifest'].includes(p)) { const a = await env.ASSETS.fetch(new Request(origin + p)); return a.ok ? new Response(a.body, { status: 200, headers: headers({ 'content-type': a.headers.get('content-type') || 'application/octet-stream', 'cache-control': 'public, max-age=86400' }) }) : json({ error: 'not_found' }, 404); }
