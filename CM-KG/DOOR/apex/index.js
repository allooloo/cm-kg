...
const OPERATOR = 'Allooloo Technologies Corp.'; const SERVER_VERSION = '0.12.1';   // ORDER-021 (Sept 16 2026): the human root renders each live node's link from nodes.json `door` (the field of record), not from a rebuilt hostname const PROTOCOL = '2025-06-18';
...
<p>Live nodes (${live.length}): ${live.map(n => `<a href="${n.door || '#'}">${n.country}</a>`).join(' · ')}</p>
