"""Shared pieces for the Japan Width 1 (Disclosure) rail (ORDER-017 re-cut). Same shape as the Swiss rail; Japan issuers and the regulator filing trail; no Tavily search layer on this node (ORDER-017: Perplexity Search + preset fast at the Fill pass, Tavily extraction only).
...
sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\jp-width0')
...
TABS = ['Prime Market', 'Standard Market', 'Growth Market', 'PRO Market', 'REITs and funds']; TICKER_COL = 'Symbol'; REG_COL = 'EDINET code'; HOME_ISIN = ''
...
def load_issuers(types=('Corporate', 'Corporate (foreign issuer)', 'REIT / listed fund')):
    ...
    for ex in TABS:
        ...
        for r in it:
            ...
            nm = d.get('Legal name (English, as published)') or d.get('Legal name (Japanese)') or ''
            out.append({'exchange': ex, 'ticker': str(d[TICKER_COL]), 'name': nm, 'name_native': d.get('Legal name (Japanese)') or '', 'full_name': (_LEIREC.get(d.get('LEI') or '') or {}).get('name') or nm, 'isin': d.get('ISIN') or '', 'lei': d.get('LEI') or '', 'reg_id': str(d.get(REG_COL) or ''),
