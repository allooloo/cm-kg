...
COUNTRY = 'Japan'; ORDER = 'ORDER-017 (re-cut)'
REG_LABEL = 'EDINET'
...
for fn, w in (('raw/edinet_events.jsonl', 'edinet_events'),):
    take(load(fn), w)
...
EXEMPT = ('EDINET', 'TDnet', 'Grok')  # the regulator's filing list and the live layer are the issuer's own rows
for e in events:
...
events = kept
def prio(e): return 0 if 'EDINET' in e['read_by'] else 2
seen = {}
...
BROKEN = [('all', '', '', 'TDnet timely disclosure', 'TDnet (JPX timely disclosure) has no public API and its search is a JavaScript form; EDINET statutory filings are the trail read here (HITL: TDnet data feed or a local partner).')]
...
HITL = [('TDnet feed', 'JPX timely disclosure (決算短信, adjustments, ad hoc) reaches machines only through the paid TDnet data service; the statutory EDINET trail is complete for securities reports, quarterly / half-year reports, extraordinary reports, large-holding reports and prospectuses.'), ('Machine translation', 'Titles are carried in Japanese as EDINET publishes them; no English rendering is produced (ORDER-017: machine-translated English is never a source).'), ('Anthropic Admin API key', 'Balance and cost reads for the spend line need an Admin API key; the Messages key answers 401.')]
...
METHOD = [('Order', f'{ORDER} {COUNTRY} Width 1: Disclosure. Window {SINCE} to {TODAY}. Scope: {len(issuers)} carried lines from {CC}-issuers.xlsx. Pond-native: inputs are every drop of POND\\{NODE}\\width1 plus the last versioned event set. Coverage is what the public sources give; nothing is padded.'),
          ...
          ('EDINET filings', 'The EDINET API v2 documents list per day over the window (type=2, keyed with AGENT KEYS\\edinet.txt), kept for the roster\'s EDINET codes: document type code, description (Japanese), filer, submission date and the EDINET viewer page.'),
          ...
          ('Languages', 'Japanese titles are carried as the regulator publishes them and labelled by language; machine-translated English is never a source; Mistral review only where it applies.'),
