"""Pass 2.5 — Perplexity Agent API (preset 'low', grounded) locates the current company page for issuers whose AFM notification registers search returned
nothing in the window or whose exchange record carried no website — the issuer's own investor-relations / AFM notification registers page — and the page is
...
def fetch(r):
    prompt = (f"Find the official investor-relations or ad hoc / press-release page for the Netherlands-listed company {r['name']} ({r['exchange']}: {r['ticker']}, legal name {r.get('full_name') or r['name']}): the company's own website page. "
...
if __name__ == '__main__':
    targets = sorted(issuers, key=lambda r: 0 if key(r) not in has_ev else 1)  # nl-issuers.xlsx carries no website column: every issuer is a target, zero-event issuers first
