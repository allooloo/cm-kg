"""Pass 2.2 — Claude adjudicates the Width 0 pockets from cited evidence, or leaves the field blank with a one-line reason. Label 'adjudicated by Claude'.
  ...
  (b) KVK number gaps: issuers without an KVK number (no LEI registeredAs at KVK, no name-exact register row). Evidence: KVK trade register (via the LEI record) candidates whose name shares the issuer's key
      tokens (public companies), the GLEIF record, the exchange short name. Ruling: the KVK number that IS the issuer (same legal entity), or blank (overseas, trust, name differs).
...
def rule_lei(r):
    ...
    ev = [f"Issuer on the exchange list: {r['name']} ({r['exchange']}: {r['ticker']}), ISIN {r['isin']}, KVK number {r.get('acn') or 'none'}.",
    ...
    a = KVK.get(r.get('reg_id') or '')
    if a: ev.append(f"KVK trade register (via the LEI record): KVK number {a['reg_id']} {a['name']} type {a.get('entity_type')} / {a.get('company_type')} status {a.get('status')}")
...
def rule_acn(r):
    ...
    cands = [KVK[c] for c in cands if all(t in norm(KVK[c]['name']) for t in kt[:2])][:8] if kt else []
    ev = [f"Issuer on the exchange list: {r['name']} ({r['exchange']}: {r['ticker']}), ISIN {r['isin']}; trading name {r.get('trading_name') or ''}; address on record: {row.get('Registered office') or ''}; GLEIF jurisdiction {rec.get('jur') or ''}, GLEIF registeredAs {rec.get('registeredAs') or ''} at {rec.get('registeredAt') or ''}.",
          "KVK candidates (public companies and foreign-company branches) sharing the issuer's name tokens: " + json.dumps([{'reg_id': c['reg_id'], 'name': c['name'], 'entity_type': c.get('entity_type'), 'company_type': c.get('company_type'), 'status': c.get('status'), 'incorporated': c.get('incorporated'), 'former': c.get('former_names', [])[:3]} for c in cands])[:3500]]
    prompt = ("Question: which KVK number is THIS listed issuer's own KVK registration? Accept only the same legal entity (its current or former name). A trustee, manager, "
              "subsidiary or a similarly named private company is NOT the issuer. Issuers incorporated outside Netherlands (Bermuda, Cayman, BVI, Hong Kong, China, Malaysia, Australia …) "
              "carry a KVK number only when registered as a foreign-company branch — say so and leave the KVK number empty when no branch row is the issuer.\n\nEVIDENCE:\n" + '\n'.join(ev)[:9000] +
              '\n\nReply JSON: {"uen": "<KVK number or empty>", "why_blank": "overseas|trust|name_differs|no_candidate|unclear|", "confidence": "high|medium|low", "reason": "<one line>"}')
    res = claude(prompt, SYS); j = jparse(res.get('text', '')) or {}
    acn = re.sub(r'[^A-Z0-9]', '', (j.get('reg_id', '') or j.get('acn', '') or '').upper())
    return {'field': 'KVK number', 'ruling': acn if acn in KVK else '', 'why_blank': j.get('why_blank', ''), 'confidence': j.get('confidence', ''), 'reason': j.get('reason', ''), 'source': KVK[acn]['src'] if acn in KVK else '', 'candidates': [c['reg_id'] for c in cands], 'usage': res.get('usage'), 'error': res.get('error')}
if __name__ == '__main__':
    lei_items = [byk[k] for k, row in rows.items() if k in byk and (row.get('LEI State') or '') == 'conflict']
    acn_items = []  # register-id gaps: no candidate index for this node (KVK / Handelsregister have no bulk file); the LEI record route stands
    print('LEI conflicts', len(lei_items), '| KVK number gaps', len(acn_items), flush=True)
