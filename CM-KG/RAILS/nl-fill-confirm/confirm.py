"""Pass 3 — Confirm. Every filled or sourced identity field gets a second, independent source where one exists:
  ...
  LEI             second = the KVK number on the LEI record (registeredAs at KVK) equals the register row, else the mapping file vs name-exact
  KVK number / ABN       second = the GLEIF record's registeredAs equals the KVK number (when the KVK number came from the name route), or Claude's ruling agrees (when from the LEI route)
  ...
  State           second = GLEIF legal-address region vs the KVK state of registration / the GLEIF address
...
EV = events(); wire_by = {}
for e in EV:
    if e.get('wire') and 'KVK' not in e.get('read_by', '') and 'Perplexity' not in e.get('read_by', ''): wire_by.setdefault(e['exchange'] + '|' + e['ticker'], Counter())[e['wire'].split(' (')[0]] += 1
_src = open(r'C:\ALLOOLOO\CM-KG\RAILS\nl-width0\enrich.py', encoding='utf-8').read()
REG_LIST = eval(re.search(r'^REG_LIST = (\[.*?\])\s*$', _src, re.M | re.S).group(1))
AUD_FIRMS = [('KPMG', r'\bKPMG\b'), ('Deloitte', r'\bDeloitte\b'), ('PricewaterhouseCoopers', r'PricewaterhouseCoopers|\bPwC\b'), ('Ernst & Young', r'Ernst\s*&\s*Young|\bEY\b'), ('BDO', r'\bBDO\b'), ('Mazars', r'Mazars'), ('Grant Thornton', r'Grant Thornton'), ('Baker Tilly', r'Baker Tilly'), ('Crowe', r'\bCrowe\b'), ('RSM', r'\bRSM\b'), ('Flynth', r'Flynth'), ('Accon', r'\bAccon\b')]
def firm_key(s):
...
def confirm_issuer(r):
    ...
    if (row.get('LEI State') or '') == 'conflict':
        ...
        acn = row.get('KVK number') or ''; ras = re.sub(r'[^A-Z0-9]', '', (rec.get('registeredAs') or '').upper())
        if rec.get('registeredAt') == ('RA000463',) and acn and ras == acn and 'name-exact' in (row.get('Register read by') or '') + (row.get('LEI read by') or ''): out['fields']['LEI'] = {'state': 'confirmed', 'second': [KVK.get(acn, {}).get('src', '')], 'read_by2': 'KVK number on the LEI record equals the name-exact register row'}
    ...
    v = row.get('KVK number') or ''; ca = claude_acn.get(k, {})
    if v:
        ras = re.sub(r'[^A-Z0-9]', '', (rec.get('registeredAs') or '').upper())
        if 'LEI record' in (row.get('Register read by') or ''):
            out['fields']['KVK number'] = {'state': 'confirmed', 'second': [ca.get('source', '')], 'read_by2': 'KVK trade register (via the LEI record) candidates adjudicated by Claude return the same KVK number'} if (ca.get('ruling') or '') == v else {'state': 'sourced', 'second': [], 'read_by2': ''}
        elif ras == v: out['fields']['KVK number'] = {'state': 'confirmed', 'second': [rec.get('src', '')], 'read_by2': 'GLEIF LEI record registeredAs equals the name-matched KVK number'}
        else: out['fields']['KVK number'] = {'state': 'sourced', 'second': [], 'read_by2': ''}
    elif ca.get('ruling'): out['fields']['KVK number'] = {'state': 'filled', 'value': ca['ruling'], 'second': [ca.get('source', '')], 'read_by2': 'adjudicated by Claude: ' + (ca.get('reason') or '')[:120]}
    # registry
    ...
    v = row.get('State') or ''
    if v:
        g_reg = (rec.get('legal_region') or ''); a = KVK.get(row.get('KVK number') or '', {})
        alt = (g_reg[3:] if g_reg.startswith('AU-') else '') if 'KVK' in (row.get('State read by') or '') or 'contact' in (row.get('State read by') or '') else (a.get('state') or '')
        if alt:
