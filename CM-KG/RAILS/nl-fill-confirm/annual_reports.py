"""Pass 2.3 input — the latest annual report per issuer, found on the issuer's own site by Tavily (query "<name> jaarverslag annual report pdf",
...
def find_report(r):
    ...
    j = tavily({'query': f"{nm} jaarverslag annual report {TODAY.year - 1} pdf", 'max_results': 10})
    best = None
    for x in j.get('results', []):
        u = x['url']
        if not re.search(r'(?i)\.pdf(\?|$)', u): continue
        if not name_match(x['title'] + ' ' + u, r['name'], (r.get('full_name'),))[0] in ('full', 'distinctive'): continue
        if not re.search(r'(?i)annual|jaarverslag|financial report|integrated report|jaarrekening', x['title'] + u): continue
