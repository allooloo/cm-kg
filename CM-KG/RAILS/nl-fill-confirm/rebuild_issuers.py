r"""Rebuilds CM-KG\ISSUERS\nl-issuers.xlsx with passes 2 and 3 applied: 'Also known as' (+ source, read by); Claude rulings written into LEI / KVK number where the
...
tab listing all eight engines with the exact duty and count on Netherlands. The Width 0 workbook is kept beside it as nl-issuers-width0.xlsx; a versioned copy
...
SRC = r'C:\ALLOOLOO\CM-KG\ISSUERS\nl-issuers.xlsx'; BAK = SRC.replace('nl-issuers.xlsx', 'nl-issuers-width0.xlsx')
...
KVK = {d['key']: d for d in pond.read_jsonl_all(NODE, 'width0', 'lei_records.jsonl', key='key')}
FIELDS = ['ISIN', 'LEI', 'KVK number', 'Share registrar', 'Auditor', 'Newswire of habit', 'Also known as']
STATECOL = {'ISIN': 'ISIN State', 'LEI': 'LEI State', 'KVK number': 'Register State', 'Share registrar': 'Share registrar State', 'Auditor': 'Auditor State', 'Newswire of habit': 'Newswire State'}
RBCOL = {'ISIN': 'ISIN read by', 'LEI': 'LEI read by', 'KVK number': 'Register read by', 'Share registrar': 'Share registrar read by', 'Auditor': 'Auditor read by', 'Newswire of habit': 'Newswire read by'}
...
for ex in TABS:
    ...
    for rr in range(2, ws.max_row + 1):
        ...
        ca = claude_acn.get(k)
        if ca is not None and not cell(rr, 'KVK number').value:
            if ca.get('ruling'):
                a = KVK.get(ca['ruling'], {}); cell(rr, 'KVK number').value = ca['ruling']; cell(rr, 'KVK link (unverified — search entry)').value = a.get('src', ca.get('source', ''))
                cell(rr, 'Register source').value = ca.get('source', ''); cell(rr, 'Register read by').value = 'KVK trade register (via the LEI record) register candidates · adjudicated by Claude'; cell(rr, 'Register State').value = 'filled'; cell(rr, 'Register State').value = a.get('status', ''); cell(rr, 'Legal form (GLEIF ELF code)').value = (a.get('entity_type') or '') + ((' — ' + a['company_type']) if a.get('company_type') and a['company_type'] != 'na' else ''); touched['Claude:KVK number ruled'] += 1
            else:
                touched['Claude:KVK number blank with reason'] += 1; gaps_new.append((ex, tick, 'KVK number', f"adjudicated by Claude, left blank ({ca.get('why_blank') or 'unclear'}): {(ca.get('reason') or '')[:140]}"))
        if rp:
...
def method_rows():
    ...
    return [('Claude (claude-sonnet-5)', f"Adjudicated from cited evidence: LEI conflicts {len(claude_lei)} ({touched['Claude:LEI ruled']} ruled, {touched['Claude:LEI blanked']} blanked); KVK number gaps {len(claude_acn)} ({touched['Claude:KVK number ruled']} ruled, {touched['Claude:KVK number blank with reason']} blank with reason). Pass 3 conflicts: {len(rulings)} cases, {touched['Claude:conflicts ruled']} ruled, {touched['Claude:conflicts blanked']} blanked. Label 'adjudicated by Claude'."),
            ...
            ('Rule', 'Aliases sourced only (KVK former names, the exchange trading name, GLEIF other names, wire page names); never a guess. States: sourced = one source read; filled = written by adjudication or a lab read; confirmed = two independent sources agree, both cited; conflict = the second source disagrees, sent to Claude or left blank with reason.')]
if 'Method' in wb.sheetnames:
    mt = wb['Method']; mt.append([]); mt.append(['Passes 2 and 3 (ORDER-016 Part B, ' + TODAY.isoformat() + ')', 'Engine duty and count of fields touched on Netherlands'])
...
wb.save(SRC); _ad = pond.assembled(NODE); shutil.copy(SRC, os.path.join(_ad, 'nl-issuers.xlsx'))
