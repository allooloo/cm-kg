...
by_iss = {}
for e in EV:
    if 'KVK' in e.get('read_by', ''): continue
...
def fetch(r):
    ...
    prompt = (f"Issuer: {r['name']} ({r['exchange']}: {r['ticker']}). Below are this issuer's AFM notification registers over the last 12 months (date | headline | link) and excerpts of its lodged documents.\n"
