"""Shared pieces for the Netherlands Fill + Confirm rail (passes 2 and 3). Same shape as the UK rail; Netherlands pockets. Pond-native:
raw\\ is a junction to the open drop POND\\nl-cm-kg\\fill-confirm\\<date>\\; jload reads every drop (newest wins per key)."""
import os, sys, re, json, time, threading, html, datetime
import requests, openpyxl
sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\nl-width1'); sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\nl-width0')
import keys
import pond
...
NODE = 'nl-cm-kg'
ISSUERS_XLSX = pond.latest_assembled(NODE, 'nl-issuers.xlsx') or r'C:\ALLOOLOO\CM-KG\ISSUERS\nl-issuers.xlsx'
EVENTS_JSONL = pond.latest_assembled(NODE, 'nl-events.jsonl') or r'C:\ALLOOLOO\CM-KG\DISCLOSURE\events\nl-events.jsonl'
