...
(Handelsregister former names, exchange short name, GLEIF other names, wire page names). A title matches when it carries every key token of the name or an alias; a title
...
amb = set()
try:
    wb = openpyxl.load_workbook(pond.latest_assembled(NODE, 'de-disclosure.xlsx') or r'C:\ALLOOLOO\CM-KG\DISCLOSURE\de-disclosure.xlsx', read_only=True); ws = wb['Gaps']; it = ws.iter_rows(values_only=True); hdr = next(it)
    for row in it:
        d = dict(zip(hdr, row))
        if d.get('Worker / field') == 'ambiguous match': amb.add(f"{d['Exchange']}|{d['Code']}")
except Exception: pass
DOMS = ['prnewswire.com', 'globenewswire.com', 'businesswire.com', 'accesswire.com', 'media-outreach.com', 'acnnewswire.com', 'eqs-news.com']
    ...
    for nm in names[:4]:
        for payload in ({'query': nm, 'max_results': 15, 'topic': 'news', 'days': 365, 'include_domains': DOMS}, {'query': f'"{nm}"', 'max_results': 15, 'include_domains': DOMS}):
            ...
            for x in j.get('results', []):
                ...
                w = wire_of(u) or ('EQS News' if 'eqs-news.com' in u else '')
                if oth:
                    t, st = page_text(u)
