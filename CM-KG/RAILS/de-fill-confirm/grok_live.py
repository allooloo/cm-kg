...
def halts():
    out = []; usage = []
    for ex, label in [('Xetra', 'Frankfurt Stock Exchange / Xetra (Deutsche Börse)')]:
...
def silent(keys_silent):
    ...
    for k in keys_silent:
        r = byk[k]
        items, res = ask(f"List EQS News (ad hoc, corporate news, voting rights, directors dealings) and press releases issued by {r['name']} (Xetra: {r['ticker']}, Germany) in the last {HOURS} hours, with the URL of each. Return JSON only: {{\"items\": [{{\"date\": \"YYYY-MM-DD\", \"title\": \"headline\", \"url\": \"URL\"}}]}}. Only real announcements with URLs; empty list if none.")
        usage.append(res.get('usage'))
        for it in items:
            u = str(it.get('url', ''))
