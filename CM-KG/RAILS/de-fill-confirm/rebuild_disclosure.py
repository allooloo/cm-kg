r"""Rebuilds CM-KG\DISCLOSURE\de-disclosure.xlsx and events\de-events.jsonl with passes 2 and 3: rematch events, Gemini AGM / record-date findings,
...
de-disclosure-width1.xlsx (+ de-events-width1.jsonl); a versioned copy lands in the pond."""
...
X = r'C:\ALLOOLOO\CM-KG\DISCLOSURE\de-disclosure.xlsx'; J = r'C:\ALLOOLOO\CM-KG\DISCLOSURE\events\de-events.jsonl'
BX = X.replace('de-disclosure.xlsx', 'de-disclosure-width1.xlsx'); BJ = J.replace('de-events.jsonl', 'de-events-width1.jsonl')
...
def fam(e):
    ...
    if 'Handelsregister' in rb: return 'registry'
...
mt = wb['Method']; mt.append([]); mt.append(['Passes 2 and 3 (ORDER-015 Part B, ' + TODAY.isoformat() + ')', 'Engine duty and count on the Germany Disclosure workbook'])
