"""Pass 2.2 — Claude adjudicates the Width 0 pockets from cited evidence, or leaves the field blank with a one-line reason. Label 'adjudicated by Claude'.
  ...
  (b) register number gaps: issuers without an register number (no LEI registeredAs at Handelsregister, no name-exact register row). Evidence: Handelsregister (HRA/HRB sheet on the LEI record) candidates whose name shares the issuer's key
      tokens (public companies), the GLEIF record, the exchange short name. Ruling: the register number that IS the issuer (same legal entity), or blank (overseas, trust, name differs).
...
def rule_lei(r):
    ...
    ev = [f"Issuer on the exchange list: {r['name']} ({r['exchange']}: {r['ticker']}), ISIN {r['isin']}, register number {r.get('acn') or 'none'}.",
    ...
    if a: ev.append(f"Handelsregister (HRA/HRB sheet on the LEI record): register number {a['reg_id']} {a['name']} type {a.get('entity_type')} / {a.get('company_type')} status {a.get('status')}")
...
def rule_acn(r):
    ...
    cands = [Handelsregister[c] for c in cands if all(t in norm(Handelsregister[c]['name']) for t in kt[:2])][:8] if kt else []
    ev = [f"Issuer on the exchange list: {r['name']} ({r['exchange']}: {r['ticker']}), ISIN {r['isin']}; trading name {r.get('trading_name') or ''}; address on record: {row.get('Registered office') or ''}; GLEIF jurisdiction {rec.get('jur') or ''}, GLEIF registeredAs {rec.get('registeredAs') or ''} at {rec.get('registeredAt') or ''}.",
          "Handelsregister candidates (public companies and foreign-company branches) sharing the issuer's name tokens: " + json.dumps([{'reg_id': c['reg_id'], 'name': c['name'], 'entity_type': c.get('entity_type'), 'company_type': c.get('company_type'), 'status': c.get('status'), 'incorporated': c.get('incorporated'), 'former': c.get('former_names', [])[:3]} for c in cands])[:3500]]
    prompt = ("Question: which register number is THIS listed issuer's own Handelsregister registration? Accept only the same legal entity (its current or former name). A trustee, manager, "
              "subsidiary or a similarly named private company is NOT the issuer. Issuers incorporated outside Germany (Bermuda, Cayman, BVI, Hong Kong, China, Malaysia, Australia …) "
              "carry a register number only when registered as a foreign-company branch — say so and leave the register number empty when no branch row is the issuer.\n\nEVIDENCE:\n" + '\n'.join(ev)[:9000] +
              '\n\nReply JSON: {"uen": "<register number or empty>", "why_blank": "overseas|trust|name_differs|no_candidate|unclear|", "confidence": "high|medium|low", "reason": "<one line>"}')
    res = claude(prompt, SYS); j = jparse(res.get('text', '')) or {}
    acn = re.sub(r'[^A-Z0-9]', '', (j.get('reg_id', '') or j.get('acn', '') or '').upper())
    return {'field': 'Register number (HR)', 'ruling': acn if acn in Handelsregister else '', 'why_blank': j.get('why_blank', ''), 'confidence': j.get('confidence', ''), 'reason': j.get('reason', ''), 'source': Handelsregister[acn]['src'] if acn in Handelsregister else '', 'candidates': [c['reg_id'] for c in cands], 'usage': res.get('usage'), 'error': res.get('error')}
if __name__ == '__main__':
    lei_items = [byk[k] for k, row in rows.items() if k in byk and (row.get('LEI State') or '') == 'conflict']
    acn_items = []  # register-id gaps: no candidate index for this node (Zefix / Handelsregister have no bulk file); the LEI record route stands
    print('LEI conflicts', len(lei_items), '| register number gaps', len(acn_items), flush=True)
