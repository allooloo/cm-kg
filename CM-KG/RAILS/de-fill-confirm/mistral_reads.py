...
expected case on Germany; the worker reports the count it found and reads exactly those, no padding. Label: 'read by Mistral (zh)'."""
...
targets = [e for e in EV if (e.get('language') or '') in ('de', 'fr', 'it') or NON_EN.search(e.get('title', ''))]  # German / French / Italian items: Mistral reads them natively (label 'read by Mistral (de)')
def fetch(e):
    ...
    return {'url': e['url'], **j, 'usage': res.get('usage'), 'error': res.get('error'), 'read_by': 'read by Mistral (de)'}
