"""Step 6 — Tavily reads for the fields no registry publishes: share registrar (closed list of Singapore registrars within 300 characters of "registrar")
and newswire of habit beyond SGXNet (majority of up to three releases on the wire domains that carry the issuer name). Auditor is NOT read here: the ACRA
register carries the audit firm(s) of record per company and the annual report link is the second source. Every accepted value keeps the source URL and an
...
STOP = {'limited', 'ltd', 'the', 'company', 'co', 'group', 'holdings', 'holding', 'trust', 'fund', 'inc', 'corp', 'corporation', 'international', 'global', 'capital', 'investments', 'investment', 'pte', 'singapore', 'and', 'technologies', 'technology', 'industries', 'engineering', 'asia', 'pacific', 'china', 'ordinary', 'shares', 'ordshs'}
...
REG_LIST = [('Boardroom Corporate & Advisory Services Pte. Ltd.', r'Boardroom'), ('Tricor Barbinder Share Registration Services', r'Tricor|Barbinder'), ('M & C Services Private Limited', r'M\s*&\s*C Services'), ('KCK CorpServe Pte. Ltd.', r'KCK Corp'), ('B.A.C.S. Private Limited', r'B\.?A\.?C\.?S\.?'), ('In.Corp Corporate Services Pte. Ltd.', r'In\.?Corp Corporate'), ('RHT Corporate Advisory', r'RHT Corporate'), ('Elite Partners', r'Elite Partners'), ('Vistra', r'Vistra'), ('Intertrust', r'Intertrust'), ('Computershare Hong Kong Investor Services', r'Computershare')]
def do_reg(r):
    j = tavily({'query': f"{r['name']} share registrar", 'max_results': 6, 'include_raw_content': True})
    ...
    for x in j.get('results', []):
        ...
        for m in re.finditer(r'(?i)\bshare regist(?:rar|ry)\b|\bregistrar\b', txt):
...
WIRES = {'prnewswire.com': 'PR Newswire', 'globenewswire.com': 'GlobeNewswire', 'businesswire.com': 'Business Wire', 'accesswire.com': 'ACCESS Newswire', 'newsfilecorp.com': 'Newsfile', 'media-outreach.com': 'Media OutReach', 'acnnewswire.com': 'ACN Newswire'}
