...
def fetch(r):
    prompt = (f"Find the official investor-relations or ad hoc / press-release page for the Tokyo Stock Exchange-listed company {r['name']} (securities code {r['ticker']}, {r['exchange']}; South Koreaese name {r.get('name_native') or ''}): the company's own website page (South Koreaese or English). "
...
if __name__ == '__main__':
    targets = sorted(issuers, key=lambda r: 0 if key(r) not in has_ev else 1)  # kr-issuers.xlsx carries no website column: every issuer is a target, zero-event issuers first
