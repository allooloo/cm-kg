r"""Rebuilds kr-issuers.xlsx with the Fill pass applied (ORDER-017 re-cut), into a NEW assembled drop beside the Width 0 workbook (mirror refreshed):
  Auditor / Share registrar / Accounts period end / Going concern ... from Gemini's South Koreaese read of the 有価証券報告書 text windows (State filled;
...
docs = {d['key']: d for d in jload('raw/dart_docs.jsonl') if d.get('document_url')}
aud = {d['key']: d for d in jload('raw/dart_auditor.jsonl') if d.get('auditor')}
...
for ex in TABS:
    ...
    for rr in range(2, ws.max_row + 1):
        ...
        if d:
            cell(rr, 'Annual report').value = d['document_url']; cell(rr, 'Annual report source').value = d['document_url']; cell(rr, 'Annual report read by').value = f"DART document API (사업보고서 receipt {d.get('docID')}, filed {d.get('doc_date')})"; touched['DART:annual report link'] += 1
        a = aud.get(k)
        if a:
            cell(rr, 'Auditor').value = a['auditor']; cell(rr, 'Auditor source').value = a['source_url']; cell(rr, 'Auditor read by').value = a['read_by']; cell(rr, 'Auditor State').value = 'sourced'; touched['DART:Auditor'] += 1
            if a.get('period_end'): cell(rr, 'Accounts period end').value = a['period_end']; touched['DART:period end'] += 1
            if a.get('going_concern'): cell(rr, 'Going concern (annual report)').value = a['going_concern']; touched['DART:going concern'] += 1
            if not d: cell(rr, 'Annual report').value = a['source_url']; cell(rr, 'Annual report source').value = a['source_url']; cell(rr, 'Annual report read by').value = 'DART list / auditor API (사업보고서 receipt ' + a.get('rcept_no', '') + ')'; touched['DART:annual report link'] += 1
        if g and not g.get('error'):
            def put(field, val, ok_key, statecol, rbcol, srccol):
                ...
                if ck.get(ok_key) is False: rb += ' · disputed on review by Mistral (ko)'; extra_gaps.append(f"{field}: Mistral review disputes the Gemini reading ({(m.get('reason') or '')[:120]})"); touched[f'Mistral:{field} disputed'] += 1
                cell(rr, rbcol).value = rb; cell(rr, statecol).value = 'filled'; touched[f'Gemini:{field}'] += 1
            if not a: put('Auditor', g.get('auditor'), 'auditor_ok', 'Auditor State', 'Auditor read by', 'Auditor source')
            ...
            if g.get('period_end') and not (a and a.get('period_end')): cell(rr, 'Accounts period end').value = g['period_end']; touched['Gemini:period end'] += 1
            if g.get('going_concern') and not (a and a.get('going_concern')): cell(rr, 'Going concern (annual report)').value = {'stated': 'material uncertainty stated (계속기업 관련 중요한 불확실성)', 'not_stated': 'none stated', 'unclear': 'unclear'}.get(g['going_concern'], g['going_concern']); touched['Gemini:going concern'] += 1
        p = pp.get(k)
...
mt.append([]); mt.append(['Fill pass (ORDER-017 re-cut, ' + TODAY.isoformat() + ')', 'Engine duty and count on the South Korea issuers workbook'])
for c in mt[mt.max_row]: c.font = BOLD
mc = json.load(open('raw/mistral_count.json')) if os.path.exists('raw/mistral_count.json') else {}
for a, b in [('EDINET (API v2)', f"Latest 有価証券報告書 per issuer downloaded as PDF and reduced to South Koreaese text windows: {len(docs)} reports; the document page is the annual report link and the source of every filled field."),
             ('Google Gemini (gemini-flash-latest)', f"Primary reader of the South Koreaese windows: auditor written {touched['Gemini:Auditor']} (agrees with an existing value {touched['Gemini:Auditor agrees']}), share-register administrator written {touched['Gemini:Share registrar']}, period end {touched['Gemini:period end']}, going-concern statement {touched['Gemini:going concern']}. Label 'read by Gemini (ko)'. Values verbatim in South Koreaese; no translation."),
...
_ad = pond.assembled(NODE); out = os.path.join(_ad, 'kr-issuers.xlsx'); wb.save(out); shutil.copy(out, r'C:\ALLOOLOO\CM-KG\ISSUERS\kr-issuers.xlsx')
