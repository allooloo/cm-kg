...
def fetch(t):
    ...
    if t['kind'] == 'gemini_doc':
        ...
        return {'kind': t['kind'], 'url': t['url'], 'key': t['key'], 'issuer_key': t['key'], 'issuer': nm, 'verdict': j.get('verdict') or ('confirm' if all(j.get(k, True) for k in ('registrar_ok', 'auditor_ok', 'period_end_ok', 'going_concern_ok')) else 'dispute'), 'reason': j.get('reason', ''), 'checks': {k: j.get(k) for k in ('registrar_ok', 'auditor_ok', 'period_end_ok', 'going_concern_ok')}, 'flags': [f for f in (j.get('flags') or []) if isinstance(f, dict) and f.get('value')], 'usage': res.get('usage'), 'model': res.get('model'), 'error': res.get('error'), 'read_by': LABEL}
    txt, st = page_text(t['url'])
    if not txt or len(txt) < 200: return {'kind': t['kind'], 'url': t['url'], 'key': t['key'], 'issuer_key': t['key'], 'issuer': nm, 'verdict': 'no_view', 'reason': f'page body not readable ({st})', 'flags': [], 'read_by': LABEL}
    ...
    res = mistral(q); j = jparse(res.get('text', '')) or {}
    return {'kind': t['kind'], 'url': t['url'], 'key': t['key'], 'issuer_key': t['key'], 'issuer': nm, 'verdict': j.get('verdict', ''), 'reason': j.get('reason', ''), 'issuer_named': j.get('issuer_named'), 'page_kind': j.get('page_kind'), 'flags': [f for f in (j.get('flags') or []) if isinstance(f, dict) and f.get('value')], 'usage': res.get('usage'), 'model': res.get('model'), 'error': res.get('error'), 'read_by': LABEL}
