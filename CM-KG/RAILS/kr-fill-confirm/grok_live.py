...
def halts():
    out = []; usage = []
    for ex, label in [('KOSPI', 'Korea Exchange KOSPI'), ('KOSDAQ', 'Korea Exchange KOSDAQ')]:
...
def silent(keys_silent):
    ...
    for k in keys_silent:
        r = byk[k]
        items, res = ask(f"List EQS News / SIX ad hoc and press releases issued by {r['name']} (SIX: {r['ticker']}, South Korea) in the last {HOURS} hours, with the URL of each. Return JSON only: {{\"items\": [{{\"date\": \"YYYY-MM-DD\", \"title\": \"headline\", \"url\": \"URL\"}}]}}. Only real announcements with URLs; empty list if none.")
        usage.append(res.get('usage'))
        for it in items:
            u = str(it.get('url', ''))
...
if __name__ == '__main__':
    ...
    w1 = pond.open_drop(NODE, 'width1') if pond.drops(NODE, 'width1') else r'C:\ALLOOLOO\CM-KG\RAILS\kr-width1\raw'
