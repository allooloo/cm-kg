r"""Rebuilds CM-KG\DISCLOSURE\kr-disclosure.xlsx and events\kr-events.jsonl with the Fill pass: Grok live-layer events, Mistral review rows on a Review tab, and a State per event: sourced · confirmed (results / AGM rows
...
kr-disclosure-width1.xlsx (+ kr-events-width1.jsonl); a versioned copy lands in the pond."""
...
X = r'C:\ALLOOLOO\CM-KG\DISCLOSURE\kr-disclosure.xlsx'; J = r'C:\ALLOOLOO\CM-KG\DISCLOSURE\events\kr-events.jsonl'
BX = X.replace('kr-disclosure.xlsx', 'kr-disclosure-width1.xlsx'); BJ = J.replace('kr-events.jsonl', 'kr-events-width1.jsonl')
...
def fam(e):
    rb = e['read_by']
    if 'DART' in rb or 'KIND' in rb: return 'registry'
...
mt = wb['Method']; mt.append([]); mt.append(['Passes 2 and 3 (ORDER-017 (re-cut), ' + TODAY.isoformat() + ')', 'Engine duty and count on the South Korea Disclosure workbook'])
...
for a, b in [('Claude (claude-sonnet-5)', f'Rematch collisions ruled from the release text: {rm_conf}. Label "adjudicated by Claude".'),
             ...
             ('Perplexity (Agent API, preset fast)', f"Located company pages: {sum(1 for d in jload('raw/perplexity_pages.jsonl') if d.get('verified'))} verified; page titles became sourced aliases used by the rematch."),
             ...
             ('Mistral (mistral-small-latest)', f"Review only (ORDER-017 re-cut: Gemini reads South Koreaese as primary, Perplexity fast is the search layer, Grok second, Mistral review only): {mc.get('reviewed', 0)} native-language items reviewed of {mc.get('n_targets', 0)} — {mc.get('confirmed', 0)} confirmed, {mc.get('disputed', 0)} disputed, {mc.get('flags', 0)} Mistral-only findings flagged on the Review tab; no event or field written. Label 'review by Mistral (ko)'."),
...
wb.save(X); _ad = pond.assembled(NODE); shutil.copy(X, os.path.join(_ad, 'kr-disclosure.xlsx')); shutil.copy(J, os.path.join(_ad, 'kr-events.jsonl'))
