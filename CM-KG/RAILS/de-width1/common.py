"""Shared pieces for the Germany Width 1 (Disclosure) rail (ORDER-015 Part B). Same shape as the Switzerland rail; German issuers and sources.
Pond-native: raw\\ is a junction to the open drop POND\\ch-cm-kg\\width1\\<date>\\ (pond_open.py); assemblers read every drop."""
import os, re, json, sys, time, threading, datetime, html
import requests, openpyxl
sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\de-width0')
...
TABS = ['Xetra']; TICKER_COL = 'Symbol'; REG_COL = 'Register number (HR)'; HOME_ISIN = 'DE'
STOP = {'limited', 'ltd', 'the', 'company', 'co', 'group', 'holdings', 'holding', 'trust', 'fund', 'inc', 'corp', 'corporation', 'international', 'global', 'capital', 'investments', 'investment', 'ag', 'sa', 'se', 'and', 'technologies', 'technology', 'industries', 'engineering', 'swiss', 'schweiz', 'suisse', 'europe', 'n', 'i', 'reg', 'br', 'pc', 'ps', 'bearer', 'registered', 'aktiengesellschaft', 'holdings', 'gruppe', 'groupe'}
    ...
    for ex in TABS:
        ...
        for r in it:
            ...
            out.append({'exchange': ex, 'ticker': d[TICKER_COL], 'name': d['Legal name'], 'full_name': (d.get('Registered office read by') and d.get('Legal name')) or d['Legal name'], 'isin': d['ISIN'] or '', 'lei': d['LEI'] or '', 'reg_id': d.get(REG_COL) or '', 'acn': d.get(REG_COL) or '',
                        'wire': (d['Newswire of habit'] or '').split(' (')[0], 'wire_urls': (d['Newswire releases seen'] or '').split('\n') if d['Newswire releases seen'] else [], 'website': d.get('Website') or '', 'legal_seat': d.get('HQ city') or ''})
    return out
...
WIRE_DOMS = {'prnewswire.com': 'PR Newswire', 'globenewswire.com': 'GlobeNewswire', 'businesswire.com': 'Business Wire', 'accesswire.com': 'ACCESS Newswire', 'accessnewswire.com': 'ACCESS Newswire', 'newsfilecorp.com': 'Newsfile', 'pressetext.com': 'pressetext', 'irw-press.com': 'IRW-Press'}
def wire_of(url):
