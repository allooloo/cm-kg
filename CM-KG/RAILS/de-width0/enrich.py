"""Step 6 — Tavily reads for the fields no registry publishes: registered-share service provider (closed list within 300 characters of "share register" /
"Aktienregister") and newswire of habit (majority of up to three releases on the wire domains that carry the issuer name; EQS News is the wire of record for
most German issuers). Auditor is NOT read here (annual report / Bundesanzeiger link only at Width 0). Every accepted value keeps the source URL and an
...
STOP = {'limited', 'ltd', 'the', 'company', 'co', 'group', 'holdings', 'holding', 'trust', 'fund', 'inc', 'corp', 'corporation', 'international', 'global', 'capital', 'investments', 'investment', 'ag', 'se', 'kgaa', 'gmbh', 'und', 'and', 'technologies', 'technology', 'industries', 'engineering', 'deutschland', 'deutsche', 'german', 'europe', 'o', 'n', 'inh', 'vz', 'st', 'aktiengesellschaft', 'na', 'on', 'vna', 'vzo', 'beteiligungs'}
...
REG_LIST = [('Link Market Services GmbH', r'Link Market Services'), ('Computershare Deutschland GmbH', r'Computershare'), ('ADEUS Aktienregister-Service-GmbH', r'ADEUS'), ('Better Orange IR & HV AG', r'Better Orange'), ('Clearstream Banking AG (registered shares)', r'Clearstream'), ('Aktienregister (in-house)', r'(?i)aktienregister')]
def do_reg(r):
    j = tavily({'query': f"{r['name']} Aktienregister share registrar", 'max_results': 6, 'include_raw_content': True})
    ...
    for x in j.get('results', []):
        ...
        for m in re.finditer(r'(?i)\bshare regist(?:rar|ry|er)\b|\bregistrar\b|\baktienregister\b', txt):
...
WIRES = {'prnewswire.com': 'PR Newswire', 'globenewswire.com': 'GlobeNewswire', 'businesswire.com': 'Business Wire', 'accesswire.com': 'ACCESS Newswire', 'newsfilecorp.com': 'Newsfile', 'eqs-news.com': 'EQS News', 'dgap.de': 'EQS News', 'pressetext.com': 'pressetext', 'irw-press.com': 'IRW-Press'}
