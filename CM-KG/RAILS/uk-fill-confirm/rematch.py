"""Pass 2.1b — re-run the wire / RNS title match for zero-event issuers and for issuers with Gaps 'ambiguous match' rows, using the legal name OR a
sourced alias (Companies House previous names, GLEIF other names, the news-page display name, the exchange display name). A title matches when it
carries every key token of the name or of an alias; a title that matches two roster issuers goes to Claude, who rules from the release text or
leaves it out. Search: Tavily over the wire domains and investegate.co.uk. Labels: 'Tavily search (wire domains) … [alias: X, source]' · 'adjudicated by Claude'."""
import re
...
amb = set()
try:
    wb = openpyxl.load_workbook(r'C:\ALLOOLOO\CM-KG\DISCLOSURE\uk-disclosure.xlsx', read_only=True); ws = wb['Gaps']; it = ws.iter_rows(values_only=True); hdr = next(it)
    for row in it:
        d = dict(zip(hdr, row))
        if d.get('Worker / field') == 'ambiguous match': amb.add(f"{d['Exchange']}|{d['Ticker']}")
except Exception: pass
DOMS = ['prnewswire.com', 'prnewswire.co.uk', 'globenewswire.com', 'businesswire.com', 'accesswire.com', 'investegate.co.uk', 'londonstockexchange.com']
INDEX = re.compile(r'globenewswire\.com/(organization|search)|prnewswire\.com/news/[^/]+/?$|accesswire\.com/newsroom|investegate\.co\.uk/(company/|$)|londonstockexchange\.com/(stock/|news\?|$)')
def match_any(title, r):
    names = [(r['name'], None)] + [(a['alias'], a) for a in ALIASES.get(key(r), [])]
    for nm, a in names:
        if name_match(title, nm)[0] == 'full': return nm, a
    return None, None
def others_matching(title, r):
    hits = []
    for o in issuers:
        if o is r: continue
        if name_match(title, o['name'])[0] == 'full': hits.append(o['name'])
    return hits[:3]
    ...
    for nm in names[:4]:
        for payload in ({'query': nm, 'max_results': 15, 'topic': 'news', 'days': 365, 'include_domains': DOMS}, {'query': f'"{nm}"', 'max_results': 15, 'include_domains': DOMS}):
            ...
            for x in j.get('results', []):
                ...
                w = wire_of(u) or ('RNS' if 'investegate' in u or 'londonstockexchange' in u else '')
                if oth:
                    t, st = page_text(u)
                ...
                seen.add(u)
                evs.append(event(r, classify(x['title']), date, x['title'], w, u, rb, wire=w))
