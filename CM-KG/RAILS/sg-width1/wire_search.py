"""Newswire releases outside SGXNet, via Tavily, for every corporate issuer (the wire of habit first, then the other wires):
...
INDEX = re.compile(r'globenewswire\.com/(organization|search)|prnewswire\.com/news/[^/]+/?$|accesswire\.com/newsroom|newsfilecorp\.com/company/|businesswire\.com/(news/home/\?|portal)|media-outreach\.com/(company|search)')
def clean(u):
    u = u.split('#')[0]; return re.sub(r'^https?://(rss|secure|www2|ml-eu|mb|en)\.', 'https://www.', u)
def short_name(n):
    s = re.sub(r'\b(Limited|Ltd\.?|Holdings|Group|Company|Co\.?|Inc\.?|Corp\.?|Corporation|Pte\.?|Bhd\.?|Berhad)\b\.?', '', n, flags=re.I)
...
def fetch(r):
    ...
    for payload, rb in queries:
        ...
        for x in j.get('results', []):
            ...
            if not name_in(x['title'], r['name'], (r.get('trading_name'),)): amb += 1; continue
            seen.add(u)
