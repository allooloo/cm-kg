...
applies the name rule to search-found wire hits, and writes CM-KG\DISCLOSURE\sg-disclosure.xlsx (Events, Coverage, Gaps, HITL, Method) and
CM-KG\DISCLOSURE\events\sg-events.jsonl, with a versioned copy under POND\sg-cm-kg\assembled\<date>\."""
...
from common import *
OUT_X = sys.argv[1] if len(sys.argv) > 1 else r'C:\ALLOOLOO\CM-KG\DISCLOSURE\sg-disclosure.xlsx'
OUT_J = sys.argv[2] if len(sys.argv) > 2 else r'C:\ALLOOLOO\CM-KG\DISCLOSURE\events\sg-events.jsonl'
...
def take(rows, worker):
    for d in rows:
        ...
        for e in d.get('events', []):
            if e.get('event_type') == 'sgxnet_announcement': e['event_type'] = classify(e.get('title', ''))  # re-classify with the current rules
            events.append(e)
        if d.get('ambiguous'): worker_notes[k].append(f"{d['ambiguous']} wire hits named another issuer (not taken)")
        if d.get('other_issuer'): worker_notes[k].append(f"{d['other_issuer']} SGXNet page hits belonged to another issuer (not taken)")
        if d.get('note'): worker_notes[k].append(d['note'])
take(load('raw/sgxnet_search.jsonl'), 'sgxnet_search'); take(load('raw/acra_events.jsonl'), 'acra_events'); take(load('raw/wire_search.jsonl'), 'wire_search')
...
n_prior = 0
_prev = pond.latest_assembled(NODE, 'sg-events.jsonl') or (OUT_J if os.path.exists(OUT_J) else None)
if _prev:
...
for e in events:
    ...
    r = byk.get(k, {}); mode, tok = name_match(e['title'], e['issuer'], tuple(ALIASES.get(k, [])) + ((r.get('trading_name'),) if r.get('trading_name') else ()))
    if mode == 'full': kept.append(e)
        ...
        ambiguous[k] += 1
        why = (f"token '{tok}' is not unique across the SG roster or is a common English word" if mode == 'ambiguous' else 'no name token in the title')
        gaps.append((k, 'ambiguous match', f"{e['date']} {e['title'][:90]} | {why} | {e['url']}"))
events = kept
def prio(e): return 0 if 'links.sgx.com' in e['read_by'] else (1 if 'ACRA' in e['read_by'] else 2)
seen = {}
...
def sheet(name, cols, rows, widths):
    ...
    return ws
ECOLS = ['Market', 'Code', 'ISIN', 'LEI', 'Issuer', 'Event type', 'Date', 'Title', 'SGXNet category', 'SGXNet reference', 'Wire', 'Source', 'URL', 'Read by', 'Detail', 'State']
erows = [[e['exchange'], e['ticker'], e['isin'], e['lei'], e['issuer'], e['event_type'], e['date'], e['title'], e.get('sgx_category', ''), e.get('reference', ''), e['wire'], e['source'], e['url'], e['read_by'], e['detail'], e.get('state', 'sourced')] for e in events]
sheet('Events', ECOLS, erows, [14, 8, 15, 22, 34, 20, 11, 60, 26, 18, 14, 30, 60, 44, 30, 10])
...
TYPES = ['sgxnet_announcement', 'results', 'agm_egm', 'dividend', 'halt_suspension', 'substantial_shareholder', 'director_change', 'name_change', 'annual_return', 'status_change', 'newswire_release']
CCOLS = ['Market', 'Code', 'ISIN', 'Issuer', 'UEN', 'Newswire of habit', 'Events (live)', 'Events (built)'] + TYPES + ['Zero events (live)', 'Notes']
n_ev = len(events) + 1; crows = []
for r in issuers:
    k = (r['exchange'], r['ticker']); i = len(crows) + 2
    crows.append([r['exchange'], r['ticker'], r['isin'], r['name'], r['uen'], r['wire'], f"=COUNTIFS(Events!$A$2:$A${n_ev},A{i},Events!$B$2:$B${n_ev},B{i})", per.get(k, 0)] + [by_type[k].get(t, 0) for t in TYPES] + [f'=IF(G{i}=0,1,0)', '; '.join(worker_notes.get(key(r), []))[:400]])
cv = sheet('Coverage', CCOLS, crows, [14, 8, 15, 34, 12, 26, 12, 12] + [12] * len(TYPES) + [12, 60])
...
GCOLS = ['Market', 'Code', 'Issuer', 'Worker / field', 'Reason']; grows = []
for k, w, why in gaps:
...
BROKEN = [('all', '', '', 'SGXNet announcements API', 'api.sgx.com/announcements answers 401 to machines (also from a browser session on sgx.com). The announcement pages on links.sgx.com are public and are read one by one; discovery depends on Tavily\'s index of that host, so coverage is thin and uneven, not a full SGXNet trail (HITL: SGX API key).'),
          ('all', '', '', 'ACRA register events', 'the monthly bulk file carries the annual return date (dated) and the current status; former names carry no date and are noted, not dated.'),
          ('all', '', '', 'Issuer IR announcement pages', 'sg-issuers.xlsx carries no website column (SGX company records refuse machines); IR pages are recovered per issuer by Perplexity in the Fill pass, not read here.')]
for b in BROKEN: grows.append(list(b))
gs = sheet('Gaps', GCOLS, grows, [14, 8, 34, 24, 110])
gs.append([]); gs.append(['TOTAL gap rows', '', '', '', f'=COUNTA(E2:E{len(grows) + 1})'])
HITL = [('SGX API key (SGXNet)', 'A full SGXNet trail per issuer (every announcement, with category and broadcast time) needs the SGX announcements API, which answers 401 without a key. Until then the trail is what Tavily indexes on links.sgx.com plus the issuer\'s own pages.'),
        ('ACRA BizFile account', 'Dated register events (name changes, officer changes, status dates) need per-UEN BizFile profiles; the bulk file carries current values and the annual return date only.'),
...
METHOD = [('Order', f'ORDER-014 Part A Singapore Width 1: Disclosure. Window {SINCE} to {TODAY}. Scope: {len(issuers)} corporate issuers from sg-issuers.xlsx (security type Corporate; REITs, business trusts and depositary receipts out). Pond-native: inputs are every drop of POND\\sg-cm-kg\\width1 plus the last versioned event set. Coverage is thin where the sources are thin; nothing is padded.'),
          ('Rule', 'Blank is blank. No event without a URL. No date read out of a headline: the event date is the SGXNet broadcast date, the register date or the release date.'),
          ('Name rule', 'A search-found wire release is attributed only when its title carries every key token of the issuer name (or of a sourced alias or the trading name), or one token unique across the SG roster and not a common English word; otherwise Gaps "ambiguous match". SGXNet pages are attributed by their own Securities line (SGX code or ISIN) and the ACRA register by UEN: exempt.'),
          ('SGXNet', 'Public announcement pages on links.sgx.com (Issuer/Manager, Securities, Announcement Title = category, Date & Time of Broadcast, Sub Title, Reference), found per issuer by Tavily (news search over the window and a general search restricted to links.sgx.com), fetched and parsed; a document hit is resolved to its page. FileOpen documents without a page carry Tavily\'s published_date.'),
...
_ad = pond.assembled(NODE); shutil.copy(OUT_X, os.path.join(_ad, 'sg-disclosure.xlsx')); shutil.copy(OUT_J, os.path.join(_ad, 'sg-events.jsonl'))
