# ORDER-015 Part B build-day chain (2026-09-11): the Fill workers in order, each logged into the open fill-confirm drop. Grok runs after rematch as the live validation layer.
...
Step 'mistral' { python mistral_reads.py }
Step 'rematch' { $env:THREADS = '4'; python rematch.py }
Step 'grok live' { $env:LIVE_HOURS = '168'; $env:LIVE_MAX_ISSUERS = '200'; python grok_live.py }
