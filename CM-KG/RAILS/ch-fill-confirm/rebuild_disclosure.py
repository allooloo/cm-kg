r"""Rebuilds CM-KG\DISCLOSURE\ch-disclosure.xlsx and events\ch-events.jsonl with passes 2 and 3: rematch events, Gemini AGM / record-date findings,
...
ch-disclosure-width1.xlsx (+ ch-events-width1.jsonl); a versioned copy lands in the pond."""
...
X = r'C:\ALLOOLOO\CM-KG\DISCLOSURE\ch-disclosure.xlsx'; J = r'C:\ALLOOLOO\CM-KG\DISCLOSURE\events\ch-events.jsonl'
BX = X.replace('ch-disclosure.xlsx', 'ch-disclosure-width1.xlsx'); BJ = J.replace('ch-events.jsonl', 'ch-events-width1.jsonl')
...
for e in events_all:
    ...
    if mm and not mm.get('error') and (mm.get('period_end') or mm.get('meeting_date') or mm.get('record_date')):
        e['detail'] = (e.get('detail', '') + '; ' if e.get('detail') else '') + 'read by Mistral: ' + ', '.join(f'{k} {mm[k]}' for k in ('period_end', 'meeting_date', 'record_date', 'auditor') if mm.get(k)); e['read_by'] += ' · read by Mistral (de/fr)'
seen = {}
...
def fam(e):
    rb = e['read_by']
    if 'eqs-news' in rb or 'SHAB' in rb: return 'exchange'
    if 'Zefix' in rb: return 'registry'
...
mt = wb['Method']; mt.append([]); mt.append(['Passes 2 and 3 (ORDER-015 Part B, ' + TODAY.isoformat() + ')', 'Engine duty and count on the Switzerland Disclosure workbook'])
...
for a, b in [('Claude (claude-sonnet-5)', f'Rematch collisions ruled from the release text: {rm_conf}. Label "adjudicated by Claude".'),
             ...
             ('Mistral (mistral-small-latest)', f"German / French / Italian items read natively: {mc.get('n_targets', 0)} found, read {min(mc.get('n_targets', 0), int(E.get('MISTRAL_MAX', '600')))}. Label 'read by Mistral (de/fr)'."),
...
wb.save(X); _ad = pond.assembled(NODE); shutil.copy(X, os.path.join(_ad, 'ch-disclosure.xlsx')); shutil.copy(J, os.path.join(_ad, 'ch-events.jsonl'))
