"""Pass 3 — Confirm. Every filled or sourced identity field gets a second, independent source where one exists:
  ...
  LEI             second = the UID on the LEI record (registeredAs at Zefix) equals the register row, else the mapping file vs name-exact
  UID / ABN       second = the GLEIF record's registeredAs equals the UID (when the UID came from the name route), or Claude's ruling agrees (when from the LEI route)
  ...
  State           second = GLEIF legal-address region vs the Zefix state of registration / the GLEIF address
...
EV = events(); wire_by = {}
for e in EV:
    if e.get('wire') and 'Zefix' not in e.get('read_by', '') and 'Perplexity' not in e.get('read_by', ''): wire_by.setdefault(e['exchange'] + '|' + e['ticker'], Counter())[e['wire'].split(' (')[0]] += 1
_src = open(r'C:\ALLOOLOO\CM-KG\RAILS\ch-width0\enrich.py', encoding='utf-8').read()
REG_LIST = eval(re.search(r'^REG_LIST = (\[.*?\])\s*$', _src, re.M | re.S).group(1))
AUD_FIRMS = [('KPMG', r'\bKPMG\b'), ('Deloitte', r'\bDeloitte\b'), ('PricewaterhouseCoopers', r'PricewaterhouseCoopers|\bPwC\b'), ('Ernst & Young', r'Ernst\s*&\s*Young|\bEY\b'), ('BDO', r'\bBDO\b'), ('Grant Thornton', r'Grant Thornton'), ('Mazars', r'Mazars'), ('OBT', r'\bOBT\b'), ('Berney', r'Berney'), ('Balmer-Etienne', r'Balmer'), ('Provida', r'Provida'), ('Treuhand- und Revisionsgesellschaft Mattig-Suter', r'Mattig'), ('Fiduciaire', r'Fiduciaire'), ('MOORE', r'\bMoore\b'), ('Crowe', r'\bCrowe\b'), ('Baker Tilly', r'Baker Tilly'), ('RSM', r'\bRSM\b'), ('Nexia', r'\bNexia\b'), ('PKF', r'\bPKF\b'), ('Kreston', r'Kreston')]
def firm_key(s):
...
def confirm_issuer(r):
    ...
    if (row.get('LEI State') or '') == 'conflict':
        ...
        acn = row.get('UID (Zefix)') or ''; ras = re.sub(r'[^A-Z0-9]', '', (rec.get('registeredAs') or '').upper())
        if rec.get('registeredAt') == ('RA000548', 'RA000549') and acn and ras == acn and 'name-exact' in (row.get('Zefix read by') or '') + (row.get('LEI read by') or ''): out['fields']['LEI'] = {'state': 'confirmed', 'second': [Zefix.get(acn, {}).get('src', '')], 'read_by2': 'UID on the LEI record equals the name-exact register row'}
    ...
    v = row.get('UID (Zefix)') or ''; ca = claude_acn.get(k, {})
    if v:
        ras = re.sub(r'[^A-Z0-9]', '', (rec.get('registeredAs') or '').upper())
        if 'LEI record' in (row.get('Zefix read by') or ''):
            out['fields']['UID (Zefix)'] = {'state': 'confirmed', 'second': [ca.get('source', '')], 'read_by2': 'Zefix federal commercial register (UID) candidates adjudicated by Claude return the same UID'} if (ca.get('ruling') or '') == v else {'state': 'sourced', 'second': [], 'read_by2': ''}
        elif ras == v: out['fields']['UID (Zefix)'] = {'state': 'confirmed', 'second': [rec.get('src', '')], 'read_by2': 'GLEIF LEI record registeredAs equals the name-matched UID'}
        else: out['fields']['UID (Zefix)'] = {'state': 'sourced', 'second': [], 'read_by2': ''}
    elif ca.get('ruling'): out['fields']['UID (Zefix)'] = {'state': 'filled', 'value': ca['ruling'], 'second': [ca.get('source', '')], 'read_by2': 'adjudicated by Claude: ' + (ca.get('reason') or '')[:120]}
    # registry
    ...
    v = row.get('State') or ''
    if v:
        g_reg = (rec.get('legal_region') or ''); a = Zefix.get(row.get('UID (Zefix)') or '', {})
        alt = (g_reg[3:] if g_reg.startswith('AU-') else '') if 'Zefix' in (row.get('State read by') or '') or 'contact' in (row.get('State read by') or '') else (a.get('state') or '')
        if alt:
