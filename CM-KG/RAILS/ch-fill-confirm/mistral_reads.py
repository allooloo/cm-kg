...
finds that no engine sourced (an auditor, a registrar, a meeting or period date in a German / French / Italian text) is a FLAG for the Review tab,
never a record value. Label: 'review by Mistral (de/fr)'. Writes raw/mistral_reads.jsonl (one row per reviewed URL)."""
from fc_common import *
LANG = 'de/fr'
LABEL = f'review by Mistral ({LANG})'
NATIVE = re.compile(r"(?i)\b(résultats|rapport annuel|assemblée générale|jahresabschluss|hauptversammlung|generalversammlung|ergebnisse|halbjahr|geschäftsbericht|umsatz|dividende|kapitalerhöhung|verwaltungsrat|vorstand|aufsichtsrat|risultati|assemblea)\b")
def native(e): return (e.get('language') or '') in ('de', 'fr', 'it') or bool(NATIVE.search(e.get('title', '')))
targets = []
for e in events():
