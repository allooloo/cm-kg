"""Pass 2.2 — Claude adjudicates the Width 0 pockets from cited evidence, or leaves the field blank with a one-line reason. Label 'adjudicated by Claude'.
  ...
  (b) UID gaps: issuers without an UID (no LEI registeredAs at Zefix, no name-exact register row). Evidence: Zefix federal commercial register (UID) candidates whose name shares the issuer's key
      tokens (public companies), the GLEIF record, the exchange short name. Ruling: the UID that IS the issuer (same legal entity), or blank (overseas, trust, name differs).
...
def rule_lei(r):
    ...
    ev = [f"Issuer on the exchange list: {r['name']} ({r['exchange']}: {r['ticker']}), ISIN {r['isin']}, UID {r.get('acn') or 'none'}.",
    ...
    a = Zefix.get(r.get('reg_id') or '')
    if a: ev.append(f"Zefix federal commercial register (UID): UID {a['reg_id']} {a['name']} type {a.get('entity_type')} / {a.get('company_type')} status {a.get('status')}")
...
def rule_acn(r):
    ...
    cands = [Zefix[c] for c in cands if all(t in norm(Zefix[c]['name']) for t in kt[:2])][:8] if kt else []
    ev = [f"Issuer on the exchange list: {r['name']} ({r['exchange']}: {r['ticker']}), ISIN {r['isin']}; trading name {r.get('trading_name') or ''}; address on record: {row.get('Registered office') or ''}; GLEIF jurisdiction {rec.get('jur') or ''}, GLEIF registeredAs {rec.get('registeredAs') or ''} at {rec.get('registeredAt') or ''}.",
          "Zefix candidates (public companies and foreign-company branches) sharing the issuer's name tokens: " + json.dumps([{'reg_id': c['reg_id'], 'name': c['name'], 'entity_type': c.get('entity_type'), 'company_type': c.get('company_type'), 'status': c.get('status'), 'incorporated': c.get('incorporated'), 'former': c.get('former_names', [])[:3]} for c in cands])[:3500]]
    prompt = ("Question: which UID is THIS listed issuer's own Zefix registration? Accept only the same legal entity (its current or former name). A trustee, manager, "
              "subsidiary or a similarly named private company is NOT the issuer. Issuers incorporated outside Switzerland (Bermuda, Cayman, BVI, Hong Kong, China, Malaysia, Australia …) "
              "carry a UID only when registered as a foreign-company branch — say so and leave the UID empty when no branch row is the issuer.\n\nEVIDENCE:\n" + '\n'.join(ev)[:9000] +
              '\n\nReply JSON: {"uen": "<UID or empty>", "why_blank": "overseas|trust|name_differs|no_candidate|unclear|", "confidence": "high|medium|low", "reason": "<one line>"}')
    res = claude(prompt, SYS); j = jparse(res.get('text', '')) or {}
    acn = re.sub(r'[^A-Z0-9]', '', (j.get('reg_id', '') or j.get('acn', '') or '').upper())
    return {'field': 'UID (Zefix)', 'ruling': acn if acn in Zefix else '', 'why_blank': j.get('why_blank', ''), 'confidence': j.get('confidence', ''), 'reason': j.get('reason', ''), 'source': Zefix[acn]['src'] if acn in Zefix else '', 'candidates': [c['reg_id'] for c in cands], 'usage': res.get('usage'), 'error': res.get('error')}
if __name__ == '__main__':
    ...
    print('LEI conflicts', len(lei_items), '| UID gaps', len(acn_items), flush=True)
