...
(Zefix former names, exchange short name, GLEIF other names, wire page names). A title matches when it carries every key token of the name or an alias; a title
...
amb = set()
try:
    wb = openpyxl.load_workbook(pond.latest_assembled(NODE, 'ch-disclosure.xlsx') or r'C:\ALLOOLOO\CM-KG\DISCLOSURE\ch-disclosure.xlsx', read_only=True); ws = wb['Gaps']; it = ws.iter_rows(values_only=True); hdr = next(it)
    for row in it:
        d = dict(zip(hdr, row))
