"""Pass 2.5 — Perplexity Agent API (preset 'low', grounded) locates the current company page for issuers whose EQS News / SIX ad hoc search returned
...
def fetch(r):
    prompt = (f"Find the official investor-relations or ad hoc / press-release page for the Switzerland-listed company {r['name']} ({r['exchange']}: {r['ticker']}, legal name {r.get('full_name') or r['name']}): the company's own website page. "
...
if __name__ == '__main__':
    targets = sorted(issuers, key=lambda r: 0 if key(r) not in has_ev else 1)  # ch-issuers.xlsx carries no website column: every issuer is a target, zero-event issuers first
