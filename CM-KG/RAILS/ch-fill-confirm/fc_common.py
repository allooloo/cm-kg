"""Shared pieces for the Switzerland Fill + Confirm rail (passes 2 and 3). Same shape as the UK rail; Switzerland pockets. Pond-native:
raw\\ is a junction to the open drop POND\\ch-cm-kg\\fill-confirm\\<date>\\; jload reads every drop (newest wins per key)."""
import os, sys, re, json, time, threading, html, datetime
import requests, openpyxl
sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\ch-width1'); sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\ch-width0')
import keys
import pond
...
NODE = 'ch-cm-kg'
ISSUERS_XLSX = pond.latest_assembled(NODE, 'ch-issuers.xlsx') or r'C:\ALLOOLOO\CM-KG\ISSUERS\ch-issuers.xlsx'
EVENTS_JSONL = pond.latest_assembled(NODE, 'ch-events.jsonl') or r'C:\ALLOOLOO\CM-KG\DISCLOSURE\events\ch-events.jsonl'
