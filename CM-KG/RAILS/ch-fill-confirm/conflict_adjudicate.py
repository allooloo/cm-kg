"""Pass 3 — Claude adjudicates the conflicts Pass 3 raised (first source vs second source) for share registry, auditor, ISIN, LEI, UID, state and
...
COL = {'Share registrar': ('Share registrar', 'Share registry source', 'Share registry read by'), 'Auditor': ('Auditor', 'Annual report (EQS News)', 'Annual report read by'), 'ISIN': ('ISIN', 'ISIN source', 'ISIN read by'), 'LEI': ('LEI', 'LEI source', 'LEI read by'), 'UID (Zefix)': ('UID (Zefix)', 'Zefix source', 'Zefix read by'), 'State': ('State', 'State source', 'State read by'), 'Newswire of habit': ('Newswire of habit', 'Newswire releases seen', 'Newswire read by')}
