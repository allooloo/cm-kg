"""Shared pieces for the France Width 1 (Disclosure) rail (ORDER-016 Part B). Same shape as the Swiss rail; France issuers and sources.
Pond-native: raw\\ is a junction to the open drop POND\\fr-cm-kg\\width1\\<date>\\ (pond_open.py); assemblers read every drop."""
import os, re, json, sys, time, threading, datetime, html
import requests, openpyxl
sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\fr-width0')
...
TABS = ['Euronext Paris', 'Euronext Growth Paris', 'Euronext Access Paris']; TICKER_COL = 'ISIN'; REG_COL = 'SIREN'; HOME_ISIN = 'FR'
    ...
    for ex in TABS:
        ...
        for r in it:
            ...
            out.append({'exchange': ex, 'ticker': d[TICKER_COL], 'name': d['Legal name'], 'full_name': (_LEIREC.get(d['LEI'] or '') or {}).get('name') or d['Legal name'], 'isin': d['ISIN'] or '', 'lei': d['LEI'] or '', 'reg_id': d.get(REG_COL) or '', 'acn': d.get(REG_COL) or '',
                        'wire': (d['Newswire of habit'] or '').split(' (')[0], 'wire_urls': (d['Newswire releases seen'] or '').split('\n') if d['Newswire releases seen'] else [], 'website': d.get('Website') or '', 'legal_seat': d.get('HQ city') or '', 'amf_token': d.get('AMF filer token (BDIF)') or ''})
    return out
...
WIRE_DOMS = {'actusnews.com': 'Actusnews', 'globenewswire.com': 'GlobeNewswire', 'businesswire.com': 'Business Wire', 'prnewswire.com': 'PR Newswire', 'accesswire.com': 'ACCESS Newswire', 'newsfilecorp.com': 'Newsfile'}
def wire_of(url):
