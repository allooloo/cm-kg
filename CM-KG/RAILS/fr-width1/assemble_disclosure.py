...
COUNTRY = {'fr': 'France', 'nl': 'Netherlands'}[CC]; ORDER = 'ORDER-016 Part B'
REG_LABEL = {'fr': 'AMF BDIF', 'nl': 'AFM'}[CC]
...
for fn, w in (('raw/bdif_events.jsonl', 'bdif_events'), ('raw/afm_events.jsonl', 'afm_events'), ('raw/wire_search.jsonl', 'wire_search')):
    take(load(fn), w)
...
EXEMPT = ('AMF BDIF', 'AFM', 'Grok')  # the regulator's registers and the live layer are the issuer's own rows
for e in events:
...
events = kept
def prio(e): return 0 if 'AMF' in e['read_by'] or 'AFM' in e['read_by'] else 2
seen = {}
...
BROKEN = {'fr': [('all', '', '', 'Euronext Paris announcements', 'live.euronext.com refuses machines (anti-bot form); regulated information is read from the AMF BDIF trail per filer token instead, plus the wires.')],
          'nl': [('all', '', '', 'Euronext Amsterdam announcements', 'live.euronext.com refuses machines (anti-bot form); the AFM notification registers are read per issuer instead, plus the wires.')]}[CC]
...
HITL = {'fr': [('Euronext data account', 'The exchange\'s own announcement and list feeds sit behind an anti-bot form; a data account would give the Paris trail directly.'), ('AMF BDIF filer tokens beyond the last 10,000 items', 'Issuers whose token was not found in the recent items need the site search (JavaScript) for their trail.'), ('Anthropic Admin API key', 'Balance and cost reads for the spend line need an Admin API key; the Messages key answers 401.')],
        'nl': [('Euronext data account', 'The exchange\'s own announcement and list feeds sit behind an anti-bot form.'), ('AFM register export', 'The AFM registers answer per keyword page; a bulk export or the AFM update service would give the full trail.'), ('KVK API', 'Register events (officers, capital, status) need the paid KVK API.'), ('Anthropic Admin API key', 'Balance and cost reads for the spend line need an Admin API key; the Messages key answers 401.')]}[CC]
...
METHOD = [('Order', f'{ORDER} {COUNTRY} Width 1: Disclosure. Window {SINCE} to {TODAY}. Scope: {len(issuers)} corporate issuers with a home-country ISIN from {CC}-issuers.xlsx. Pond-native: inputs are every drop of POND\\{NODE}\\width1 plus the last versioned event set. Coverage is what the public sources give; nothing is padded.'),
          ...
          ({'fr': 'AMF BDIF', 'nl': 'AFM registers'}[CC], {'fr': 'The AMF regulatory-information database API per filer token (Jetons=RS…, 12-month window): every item with its types (financial information, documents, offers, visas, …), publication date and its BDIF page.', 'nl': 'The AFM notification registers per issuer keyword (inside information, financial reporting, directors and supervisory board transactions, substantial holdings, net short positions, prospectuses, public offers): rows with date, issuer and subject, the register detail page as URL.'}[CC]),
          ('Newswire (search)', 'Tavily news search over the wire domains with published_date; a general "<short name>" announces query; date from published_date or the release URL.'),
          ('Languages', 'French / Dutch items are kept and labelled by language; the Fill pass reads them with Mistral in review mode only (CEO rule: Perplexity → Grok → Mistral).'),
