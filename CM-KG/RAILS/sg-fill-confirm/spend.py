...
estimates; the counts are exact; Perplexity is metered (usage.cost). Written to raw/spend.json and appended to CM-KG\SPEND\sg-cm-kg-<yyyy-mm>.jsonl."""
...
out['ChatGPT (gpt-4.1-mini, Batch)'] = {'calls': ur.get('n', 0), 'input_tokens': ti, 'output_tokens': to, 'est_usd': round(ti * 0.20 / 1e6 + to * 0.80 / 1e6, 2), 'note': 'batch pricing (50% of list); text windows from SGXNet PDFs'}
...
with open(r'C:\ALLOOLOO\CM-KG\SPEND\sg-cm-kg-' + TODAY.strftime('%Y-%m') + '.jsonl', 'a', encoding='utf-8') as f: f.write(json.dumps({'date': TODAY.isoformat(), 'order': 'ORDER-014 Part B', 'spend': out}) + '\n')
