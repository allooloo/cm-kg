"""Pass 2.5 — Perplexity Agent API (preset 'low', grounded) locates the current company page for issuers whose SGXNet announcements search returned
nothing in the window or whose exchange record carried no website — the issuer's own investor-relations / SGXNet announcements page — and the page is
...
def fetch(r):
    prompt = (f"Find the official investor-relations or announcements page for the SGX-listed company {r['name']} (SGX code {r['ticker']}, trading name {r.get('trading_name') or r['name']}): the company's own website page (not sgx.com). "
...
if __name__ == '__main__':
    targets = sorted(issuers, key=lambda r: 0 if key(r) not in has_ev else 1)  # sg-issuers.xlsx carries no website column: every issuer is a target, zero-event issuers first
