r"""Rebuilds CM-KG\DISCLOSURE\sg-disclosure.xlsx and events\sg-events.jsonl with passes 2 and 3: rematch events, Gemini AGM / record-date findings,
...
seen on a second source family — SGX vs annual report vs wire — within 45 days) · filled. The Width 1 workbook is kept beside it as
sg-disclosure-width1.xlsx (+ sg-events-width1.jsonl); a versioned copy lands in the pond."""
...
X = r'C:\ALLOOLOO\CM-KG\DISCLOSURE\sg-disclosure.xlsx'; J = r'C:\ALLOOLOO\CM-KG\DISCLOSURE\events\sg-events.jsonl'
BX = X.replace('sg-disclosure.xlsx', 'sg-disclosure-width1.xlsx'); BJ = J.replace('sg-events.jsonl', 'sg-events-width1.jsonl')
...
for e in events_all:
    ...
    m = re.search(r'corporate-announcements/([A-Z0-9]{16})', e.get('url', '')); x = rep.get(m.group(1)) if m else None
...
def fam(e):
    rb = e['read_by']
    if 'links.sgx.com' in rb: return 'exchange'
    if 'ACRA' in rb: return 'registry'
...
for e in events_all:
    ws.append([CTRL.sub('', str(v)) if isinstance(v, str) else v for v in [e['exchange'], e['ticker'], e['isin'], e['lei'], e['issuer'], e['event_type'], e['date'], e['title'], e.get('sgx_category', ''), e.get('reference', ''), e['wire'], e['source'], e['url'], e['read_by'], e['detail'], e.get('state', 'sourced')]])
...
mt = wb['Method']; mt.append([]); mt.append(['Passes 2 and 3 (ORDER-014 Part B, ' + TODAY.isoformat() + ')', 'Engine duty and count on the Singapore Disclosure workbook'])
...
for a, b in [('Claude (claude-sonnet-5)', f'Rematch collisions ruled from the release text: {rm_conf}. Label "adjudicated by Claude".'),
             ('ChatGPT (gpt-4.1-mini, Batch API)', f"Read {ur.get('n', 0)} SGX-lodged documents; results-announcement period end and statement date written to Detail. Label 'read by ChatGPT (batch)'."),
...
wb.save(X); _ad = pond.assembled(NODE); shutil.copy(X, os.path.join(_ad, 'sg-disclosure.xlsx')); shutil.copy(J, os.path.join(_ad, 'sg-events.jsonl'))
