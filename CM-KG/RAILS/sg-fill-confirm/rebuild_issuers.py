r"""Rebuilds CM-KG\ISSUERS\sg-issuers.xlsx with passes 2 and 3 applied: 'Also known as' (+ source, read by); Claude rulings written into LEI / UEN where the
...
tab listing all eight engines with the exact duty and count on Singapore. The Width 0 workbook is kept beside it as sg-issuers-width0.xlsx; a versioned copy
lands in POND\sg-cm-kg\assembled\<date>\."""
import shutil
...
SRC = r'C:\ALLOOLOO\CM-KG\ISSUERS\sg-issuers.xlsx'; BAK = SRC.replace('sg-issuers.xlsx', 'sg-issuers-width0.xlsx')
...
FIELDS = ['ISIN', 'LEI', 'UEN', 'Share registrar', 'Auditor', 'Newswire of habit', 'Also known as']
STATECOL = {'ISIN': 'ISIN State', 'LEI': 'LEI State', 'UEN': 'ACRA State', 'Share registrar': 'Share registrar State', 'Auditor': 'Auditor State', 'Newswire of habit': 'Newswire State'}
RBCOL = {'ISIN': 'ISIN read by', 'LEI': 'LEI read by', 'UEN': 'ACRA read by', 'Share registrar': 'Share registrar read by', 'Auditor': 'Auditor read by', 'Newswire of habit': 'Newswire read by'}
...
for ex in TABS:
    ...
    for rr in range(2, ws.max_row + 1):
        k = ex + '|' + str(cell(rr, 'SGX code').value); d = conf.get(k, {}); c = d.get('fields', {}); rp = d.get('report') or {}; tick = k.split('|')[1]
        ...
        ca = claude_acn.get(k)
        if ca is not None and not cell(rr, 'UEN').value:
            if ca.get('ruling'):
                a = ACRA.get(ca['ruling'], {}); cell(rr, 'UEN').value = ca['ruling']; cell(rr, 'ACRA link (unverified — search entry)').value = a.get('src', ca.get('source', ''))
                cell(rr, 'ACRA source').value = ca.get('source', ''); cell(rr, 'ACRA read by').value = 'ACRA bulk register candidates · adjudicated by Claude'; cell(rr, 'ACRA State').value = 'filled'; cell(rr, 'ACRA status').value = a.get('status', ''); cell(rr, 'ACRA entity type').value = (a.get('entity_type') or '') + ((' — ' + a['company_type']) if a.get('company_type') and a['company_type'] != 'na' else ''); touched['Claude:UEN ruled'] += 1
            else:
                touched['Claude:UEN blank with reason'] += 1; gaps_new.append((ex, tick, 'UEN', f"adjudicated by Claude, left blank ({ca.get('why_blank') or 'unclear'}): {(ca.get('reason') or '')[:140]}"))
        if rp:
...
def method_rows():
    ...
    return [('Claude (claude-sonnet-5)', f"Adjudicated from cited evidence: LEI conflicts {len(claude_lei)} ({touched['Claude:LEI ruled']} ruled, {touched['Claude:LEI blanked']} blanked); UEN gaps {len(claude_acn)} ({touched['Claude:UEN ruled']} ruled, {touched['Claude:UEN blank with reason']} blank with reason). Pass 3 conflicts: {len(rulings)} cases, {touched['Claude:conflicts ruled']} ruled, {touched['Claude:conflicts blanked']} blanked. Label 'adjudicated by Claude'."),
            ('ChatGPT (gpt-4.1-mini, Batch API)', f"Read text windows from {ur.get('n', 0)} SGX-lodged documents ({n_rep} issuers: annual reports and results announcements): auditor written {touched['ChatGPT:Auditor']}, share registry {touched['ChatGPT:Registrar']}, period end {touched['ChatGPT:period end']}, going-concern flag {touched['ChatGPT:going concern']}. Label 'read by ChatGPT (batch)'."),
            ...
            ('Rule', 'Aliases sourced only (ACRA former names, the SGX trading name, GLEIF other names, wire page names); never a guess. States: sourced = one source read; filled = written by adjudication or a lab read; confirmed = two independent sources agree, both cited; conflict = the second source disagrees, sent to Claude or left blank with reason.')]
if 'Method' in wb.sheetnames:
    mt = wb['Method']; mt.append([]); mt.append(['Passes 2 and 3 (ORDER-014 Part B, ' + TODAY.isoformat() + ')', 'Engine duty and count of fields touched on Singapore'])
...
wb.save(SRC); _ad = pond.assembled(NODE); shutil.copy(SRC, os.path.join(_ad, 'sg-issuers.xlsx'))
