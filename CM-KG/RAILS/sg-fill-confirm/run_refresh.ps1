# CM-KG rail — Singapore passes 2 (Fill) and 3 (Confirm), run after the Width 1 weekly refresh (see CM-KG\RAILS\sg-weekly.ps1). Pond-native; skips on the node lock.
$ErrorActionPreference = 'Continue'
Set-Location $PSScriptRoot
if (Test-Path 'C:\ALLOOLOO\CM-KG\POND\sg-cm-kg\.lock') { 'sg-cm-kg locked (build order in flight): refresh skipped'; exit 0 }
python ..\pond_open.py sg-cm-kg sg-fill-confirm fill-confirm
...
Step 'Width 1 assemble with the live layer' { Push-Location ..\sg-width1; $env:WINDOW_DAYS = '7'; python assemble_disclosure.py; Pop-Location }
