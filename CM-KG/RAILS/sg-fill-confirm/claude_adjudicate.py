"""Pass 2.2 — Claude adjudicates the Width 0 pockets from cited evidence, or leaves the field blank with a one-line reason. Label 'adjudicated by Claude'.
  (a) LEI conflicts: the GLEIF mapping file mapped the ISIN to an LEI whose legal name is another entity. Evidence: that LEI's record, GLEIF candidates for the
      issuer's own name, the ACRA row. Ruling: the LEI that IS the issuer, or blank.
  (b) UEN gaps: issuers without an UEN (no LEI registeredAs at ACRA, no name-exact ACRA row). Evidence: ACRA bulk candidates whose name shares the issuer's key
      tokens (public companies), the GLEIF record, the SGX display name. Ruling: the UEN that IS the issuer (same legal entity), or blank (overseas, trust, name differs).
...
def rule_lei(r):
    ...
    ev = [f"Issuer on the exchange list: {r['name']} ({r['exchange']}: {r['ticker']}), ISIN {r['isin']}, UEN {r.get('acn') or 'none'}.",
    ...
    if a: ev.append(f"ACRA register: UEN {a['uen']} {a['name']} type {a.get('entity_type')} / {a.get('company_type')} status {a.get('status')}")
...
toks_index = {}
for uen, a in ACRA.items():
    for t in set(w for w in norm(a['name']).split() if w not in STOP and len(w) >= 4): toks_index.setdefault(t, set()).add(uen)
def rule_acn(r):
    ...
    cands = [ACRA[c] for c in cands if all(t in norm(ACRA[c]['name']) for t in kt[:2])][:8] if kt else []
    ev = [f"Issuer on the exchange list: {r['name']} ({r['exchange']}: {r['ticker']}), ISIN {r['isin']}; trading name {r.get('trading_name') or ''}; address on record: {row.get('Registered office') or ''}; GLEIF jurisdiction {rec.get('jur') or ''}, GLEIF registeredAs {rec.get('registeredAs') or ''} at {rec.get('registeredAt') or ''}.",
          "ACRA candidates (public companies and foreign-company branches) sharing the issuer's name tokens: " + json.dumps([{'uen': c['uen'], 'name': c['name'], 'entity_type': c.get('entity_type'), 'company_type': c.get('company_type'), 'status': c.get('status'), 'incorporated': c.get('incorporated'), 'former': c.get('former_names', [])[:3]} for c in cands])[:3500]]
    prompt = ("Question: which UEN is THIS listed issuer's own ACRA registration? Accept only the same legal entity (its current or former name). A trustee, manager, "
              "subsidiary or a similarly named private company is NOT the issuer. Issuers incorporated outside Singapore (Bermuda, Cayman, BVI, Hong Kong, China, Malaysia, Australia …) "
              "carry a UEN only when registered as a foreign-company branch — say so and leave the UEN empty when no branch row is the issuer.\n\nEVIDENCE:\n" + '\n'.join(ev)[:9000] +
              '\n\nReply JSON: {"uen": "<UEN or empty>", "why_blank": "overseas|trust|name_differs|no_candidate|unclear|", "confidence": "high|medium|low", "reason": "<one line>"}')
    res = claude(prompt, SYS); j = jparse(res.get('text', '')) or {}
    acn = re.sub(r'[^A-Z0-9]', '', (j.get('uen', '') or j.get('acn', '') or '').upper())
    return {'field': 'UEN', 'ruling': acn if acn in ACRA else '', 'why_blank': j.get('why_blank', ''), 'confidence': j.get('confidence', ''), 'reason': j.get('reason', ''), 'source': ACRA[acn]['src'] if acn in ACRA else '', 'candidates': [c['uen'] for c in cands], 'usage': res.get('usage'), 'error': res.get('error')}
if __name__ == '__main__':
    lei_items = [byk[k] for k, row in rows.items() if k in byk and (row.get('LEI State') or '') == 'conflict']
    acn_items = [byk[k] for k, row in rows.items() if k in byk and not row.get('UEN')]
    print('LEI conflicts', len(lei_items), '| UEN gaps', len(acn_items), flush=True)
