"""Pass 3 — Confirm. Every filled or sourced identity field gets a second, independent source where one exists:
  ...
  LEI             second = the UEN on the LEI record (registeredAs at ACRA) equals the ACRA row, else the mapping file vs name-exact
  UEN / ABN       second = the GLEIF record's registeredAs equals the UEN (when the UEN came from the name route), or Claude's ruling agrees (when from the LEI route)
  ...
  State           second = GLEIF legal-address region vs the ACRA state of registration / the SGX contact address
...
rows = {r['_tab'] + '|' + r['SGX code']: r for r in all_rows()}
...
EV = events(); wire_by = {}
for e in EV:
    if e.get('wire') and 'ACRA' not in e.get('read_by', '') and 'Perplexity' not in e.get('read_by', ''): wire_by.setdefault(e['exchange'] + '|' + e['ticker'], Counter())[e['wire'].split(' (')[0]] += 1
_src = open(r'C:\ALLOOLOO\CM-KG\RAILS\sg-width0\enrich.py', encoding='utf-8').read()
REG_LIST = eval(re.search(r'^REG_LIST = (\[.*?\])\s*$', _src, re.M | re.S).group(1))
AUD_FIRMS = [('KPMG', r'\bKPMG\b'), ('Deloitte', r'\bDeloitte\b'), ('PricewaterhouseCoopers', r'PricewaterhouseCoopers|\bPwC\b'), ('Ernst & Young', r'Ernst\s*&\s*Young|\bEY\b'), ('BDO', r'\bBDO\b'), ('RSM', r'\bRSM\b'), ('Baker Tilly', r'Baker Tilly'), ('Nexia', r'\bNexia\b'), ('Moore Stephens', r'\bMoore\b'), ('Crowe', r'\bCrowe\b'), ('Forvis Mazars', r'Mazars'), ('PKF', r'\bPKF\b'), ('Foo Kon Tan', r'Foo Kon Tan'), ('CLA Global TS', r'CLA Global|\bTS LLP\b'), ('RT LLP', r'\bRT LLP\b'), ('Entrust', r'Entrust Public'), ('Fiducia', r'Fiducia'), ('Ardent', r'Ardent Associates'), ('Grant Thornton', r'Grant Thornton'), ('Ecovis', r'Ecovis'), ('Kreston', r'Kreston'), ('UHY', r'\bUHY\b'), ('HLB', r'\bHLB\b'), ('Cheng & Co', r'Cheng\s*&\s*Co'), ('Prudential Public Accounting', r'Prudential Public')]
def firm_key(s):
...
def confirm_issuer(r):
    ...
    if (row.get('LEI State') or '') == 'conflict':
        ...
        acn = row.get('UEN') or ''; ras = re.sub(r'[^A-Z0-9]', '', (rec.get('registeredAs') or '').upper())
        if rec.get('registeredAt') == 'RA000523' and acn and ras == acn and 'name-exact' in (row.get('ACRA read by') or '') + (row.get('LEI read by') or ''): out['fields']['LEI'] = {'state': 'confirmed', 'second': [ACRA.get(acn, {}).get('src', '')], 'read_by2': 'UEN on the LEI record equals the name-exact ACRA row'}
    ...
    v = row.get('UEN') or ''; ca = claude_acn.get(k, {})
    if v:
        ras = re.sub(r'[^A-Z0-9]', '', (rec.get('registeredAs') or '').upper())
        if 'LEI record' in (row.get('ACRA read by') or ''):
            out['fields']['UEN'] = {'state': 'confirmed', 'second': [ca.get('source', '')], 'read_by2': 'ACRA bulk candidates adjudicated by Claude return the same UEN'} if (ca.get('ruling') or '') == v else {'state': 'sourced', 'second': [], 'read_by2': ''}
        elif ras == v: out['fields']['UEN'] = {'state': 'confirmed', 'second': [rec.get('src', '')], 'read_by2': 'GLEIF LEI record registeredAs equals the name-matched UEN'}
        else: out['fields']['UEN'] = {'state': 'sourced', 'second': [], 'read_by2': ''}
    elif ca.get('ruling'): out['fields']['UEN'] = {'state': 'filled', 'value': ca['ruling'], 'second': [ca.get('source', '')], 'read_by2': 'adjudicated by Claude: ' + (ca.get('reason') or '')[:120]}
    # registry
    ...
    if v and c:
        ...
        if (top == 'SGXNet' and v.startswith('SGXNet')) or norm(top).split()[0] == norm(v).split()[0]: out['fields']['Newswire of habit'] = {'state': 'confirmed', 'second': [f'Width 1 events: {n} of {sum(c.values())} announcements via {top}'], 'read_by2': 'Width 1 12-month event set (independent observation)'}
    ...
    v = row.get('State') or ''
    if v:
        g_reg = (rec.get('legal_region') or ''); a = ACRA.get(row.get('UEN') or '', {})
        alt = (g_reg[3:] if g_reg.startswith('AU-') else '') if 'ACRA' in (row.get('State read by') or '') or 'contact' in (row.get('State read by') or '') else (a.get('state') or '')
        if alt:
