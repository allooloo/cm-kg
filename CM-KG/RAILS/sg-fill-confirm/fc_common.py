"""Shared pieces for the Singapore Fill + Confirm rail (passes 2 and 3). Same shape as the UK rail; Singapore pockets. Pond-native:
raw\\ is a junction to the open drop POND\\sg-cm-kg\\fill-confirm\\<date>\\; jload reads every drop (newest wins per key)."""
import os, sys, re, json, time, threading, html, datetime
import requests, openpyxl
sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\sg-width1'); sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\sg-width0')
import keys
import pond
from common import norm, keytoks, to_iso, url_date, get, tavily as _tavily_raw, wire_of, classify, load_issuers, key, TODAY, name_match, _common_words, _unique_tokens, STOP, lock, UA, TH, TABS, event
NODE = 'sg-cm-kg'
ISSUERS_XLSX = pond.latest_assembled(NODE, 'sg-issuers.xlsx') or r'C:\ALLOOLOO\CM-KG\ISSUERS\sg-issuers.xlsx'
EVENTS_JSONL = pond.latest_assembled(NODE, 'sg-events.jsonl') or r'C:\ALLOOLOO\CM-KG\DISCLOSURE\events\sg-events.jsonl'
...
def sgx_docs(page_url, want=None):
    """an SGXNet announcement page on links.sgx.com lists its attachments (links.sgx.com/1.0.0/corporate-announcements/<ID>/<file>.pdf); returns [(pdf_url, name)] (filtered by the 'want' regex when given)"""
    ...
    for m in re.finditer(r'<a[^>]+href="([^"]+)"[^>]*>(.*?)</a>', x.text, re.S):
        ...
        u = h if h.startswith('http') else 'https://links.sgx.com' + h
        if want and not re.search(want, nm + ' ' + h, re.I): continue
        if u not in [o[0] for o in out]: out.append((u, nm))
    return out
def asx_pdf(viewer_url):
    """the SGX viewer (displayAnnouncement.do?display=pdf&idsId=…) answers HTML with the announcement PDF link (announcements.asx.com.au/asxpdf/…); returns (pdf_url, bytes) or (None, None)"""
