"""Pass 3 — Claude adjudicates the conflicts Pass 3 raised (first source vs second source) for share registry, auditor, ISIN, LEI, UEN, state and
...
COL = {'Share registrar': ('Share registrar', 'Share registry source', 'Share registry read by'), 'Auditor': ('Auditor', 'Annual report (SGXNet)', 'Annual report read by'), 'ISIN': ('ISIN', 'ISIN source', 'ISIN read by'), 'LEI': ('LEI', 'LEI source', 'LEI read by'), 'UEN': ('UEN', 'ACRA source', 'ACRA read by'), 'State': ('State', 'State source', 'State read by'), 'Newswire of habit': ('Newswire of habit', 'Newswire releases seen', 'Newswire read by')}
...
def rule(it):
    ...
    note = {'Newswire of habit': "Note: reading B is the channel carrying the majority of this issuer's announcements over 12 months; SGXNet is every SGX issuer's primary channel.\n",
            'Auditor': "Note: reading A comes from the independent auditor's report in the latest annual report lodged on SGXNet (the most authoritative source); reading B is page text.\n"}.get(it['field'], '')
