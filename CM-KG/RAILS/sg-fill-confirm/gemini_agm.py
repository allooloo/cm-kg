"""Pass 2.4 — Gemini (gemini-flash-latest, long context) reads each issuer's 12-month announcement set (headlines with dates and links; SGX headlines are
...
by_iss = {}
for e in EV:
    if 'ACRA' in e.get('read_by', ''): continue
...
def fetch(r):
    ...
    prompt = (f"Issuer: {r['name']} ({r['exchange']}: {r['ticker']}). Below are this issuer's SGXNet announcements over the last 12 months (date | headline | link) and excerpts of its lodged documents.\n"
