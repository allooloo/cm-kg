"""Pass 2.3 — ChatGPT (Responses API via the Batch API, model gpt-4.1-mini, strict JSON schema). One batch over text windows cut from the SGXNet PDFs
...
SCHEMA = {'type': 'object', 'additionalProperties': False, 'required': ['auditor', 'registry', 'period_end', 'statement_date', 'going_concern', 'evidence'],
          'properties': {'auditor': {'type': 'string', 'description': "audit firm that signed the independent auditor's report, proper name, or empty"},
                         'registry': {'type': 'string', 'description': 'share registrar named (e.g. Boardroom Corporate & Advisory Services Pte. Ltd., Tricor Barbinder Share Registration Services, M & C Services Private Limited, B.A.C.S. Private Limited, KCK CorpServe Pte. Ltd.), or empty'},
...
SYS = ('You extract facts from excerpts of one SGX-lodged document (annual report or results announcements). Answer only from the text. Empty string when the excerpts do not state it. '
...
def submit():
    ...
    for f in os.listdir('raw/reports'):
        ...
        lines.append(json.dumps({'custom_id': d['idsId'], 'method': 'POST', 'url': '/v1/responses', 'body': {'model': MODEL, 'input': [{'role': 'system', 'content': SYS}, {'role': 'user', 'content': f"Issuer: {d.get('key')}. Document: {d.get('kind')} {d.get('headline')} ({d.get('date')}).\n\n{text}"}],
                                 'text': {'format': {'type': 'json_schema', 'name': 'sgx_doc_extract', 'strict': True, 'schema': SCHEMA}}, 'max_output_tokens': 400}}))
