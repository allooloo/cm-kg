...
estimates; the counts are exact; Perplexity is metered (usage.cost). Written to raw/spend.json and appended to CM-KG\SPEND\jp-cm-kg-<yyyy-mm>.jsonl."""
...
with open(r'C:\ALLOOLOO\CM-KG\SPEND\jp-cm-kg-' + TODAY.strftime('%Y-%m') + '.jsonl', 'a', encoding='utf-8') as f: f.write(json.dumps({'date': TODAY.isoformat(), 'order': 'ORDER-017 (re-cut)', 'spend': out}) + '\n')
