...
while (-not ((Select-String -Path raw\build.log -Pattern 'GEMINI DONE' -Quiet) -and (Select-String -Path raw\build.log -Pattern 'CHAIN A DONE' -Quiet) -and (Select-String -Path raw\build.log -Pattern 'XBRL DONE' -Quiet))) { Start-Sleep 60 }
