...
def fetch(t):
    ...
    if t['kind'] == 'gemini_doc':
        ...
        return {'kind': t['kind'], 'url': t['url'], 'key': t['key'], 'issuer_key': t['key'], 'issuer': nm, 'verdict': j.get('verdict') or ('confirm' if all(j.get(k, True) for k in ('auditor_ok', 'registrar_ok', 'period_end_ok', 'going_concern_ok')) else 'dispute'), 'reason': j.get('reason', ''), 'checks': {k: j.get(k) for k in ('auditor_ok', 'registrar_ok', 'period_end_ok', 'going_concern_ok')}, 'flags': [f for f in (j.get('flags') or []) if isinstance(f, dict) and f.get('value')], 'usage': res.get('usage'), 'model': res.get('model'), 'error': res.get('error'), 'read_by': LABEL}
