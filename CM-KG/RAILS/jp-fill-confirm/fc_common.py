"""Shared pieces for the Japan Fill + Confirm rail (passes 2 and 3). Same shape as the UK rail; Japan pockets. Pond-native:
raw\\ is a junction to the open drop POND\\jp-cm-kg\\fill-confirm\\<date>\\; jload reads every drop (newest wins per key)."""
import os, sys, re, json, time, threading, html, datetime
import requests, openpyxl
sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\jp-width1'); sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\jp-width0')
import keys
import pond
...
NODE = 'jp-cm-kg'
ISSUERS_XLSX = pond.latest_assembled(NODE, 'jp-issuers.xlsx') or r'C:\ALLOOLOO\CM-KG\ISSUERS\jp-issuers.xlsx'
EVENTS_JSONL = pond.latest_assembled(NODE, 'jp-events.jsonl') or r'C:\ALLOOLOO\CM-KG\DISCLOSURE\events\jp-events.jsonl'
