r"""Rebuilds CM-KG\DISCLOSURE\jp-disclosure.xlsx and events\jp-events.jsonl with the Fill pass: Grok live-layer events, Mistral review rows on a Review tab, and a State per event: sourced · confirmed (results / AGM rows
...
jp-disclosure-width1.xlsx (+ jp-events-width1.jsonl); a versioned copy lands in the pond."""
...
X = r'C:\ALLOOLOO\CM-KG\DISCLOSURE\jp-disclosure.xlsx'; J = r'C:\ALLOOLOO\CM-KG\DISCLOSURE\events\jp-events.jsonl'
BX = X.replace('jp-disclosure.xlsx', 'jp-disclosure-width1.xlsx'); BJ = J.replace('jp-events.jsonl', 'jp-events-width1.jsonl')
...
def fam(e):
    rb = e['read_by']
    if 'EDINET' in rb or 'TDnet' in rb: return 'registry'
...
mt = wb['Method']; mt.append([]); mt.append(['Passes 2 and 3 (ORDER-017 (re-cut), ' + TODAY.isoformat() + ')', 'Engine duty and count on the Japan Disclosure workbook'])
...
for a, b in [('Claude (claude-sonnet-5)', f'Rematch collisions ruled from the release text: {rm_conf}. Label "adjudicated by Claude".'),
             ...
             ('Mistral (mistral-small-latest)', f"Review only (ORDER-017 re-cut: Gemini reads Japanese as primary, Perplexity fast is the search layer, Grok second, Mistral review only): {mc.get('reviewed', 0)} native-language items reviewed of {mc.get('n_targets', 0)} — {mc.get('confirmed', 0)} confirmed, {mc.get('disputed', 0)} disputed, {mc.get('flags', 0)} Mistral-only findings flagged on the Review tab; no event or field written. Label 'review by Mistral (ja)'."),
...
wb.save(X); _ad = pond.assembled(NODE); shutil.copy(X, os.path.join(_ad, 'jp-disclosure.xlsx')); shutil.copy(J, os.path.join(_ad, 'jp-events.jsonl'))
