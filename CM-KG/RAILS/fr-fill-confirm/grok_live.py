...
def halts():
    out = []; usage = []
    for ex, label in [('Euronext Paris', 'Euronext Paris'), ('Euronext Growth Paris', 'Euronext Growth Paris')]:
...
def silent(keys_silent):
    ...
    for k in keys_silent:
        r = byk[k]
        items, res = ask(f"List AMF BDIF regulated information and press releases issued by {r['name']} (SIX: {r['ticker']}, France) in the last {HOURS} hours, with the URL of each. Return JSON only: {{\"items\": [{{\"date\": \"YYYY-MM-DD\", \"title\": \"headline\", \"url\": \"URL\"}}]}}. Only real announcements with URLs; empty list if none.")
        usage.append(res.get('usage'))
        for it in items:
            u = str(it.get('url', ''))
...
if __name__ == '__main__':
    ...
    w1 = pond.open_drop(NODE, 'width1') if pond.drops(NODE, 'width1') else r'C:\ALLOOLOO\CM-KG\RAILS\fr-width1\raw'
