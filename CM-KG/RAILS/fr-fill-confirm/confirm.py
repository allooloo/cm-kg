"""Pass 3 — Confirm. Every filled or sourced identity field gets a second, independent source where one exists:
  ...
  LEI             second = the SIREN on the LEI record (registeredAs at RCS) equals the register row, else the mapping file vs name-exact
  SIREN / ABN       second = the GLEIF record's registeredAs equals the SIREN (when the SIREN came from the name route), or Claude's ruling agrees (when from the LEI route)
  ...
  State           second = GLEIF legal-address region vs the RCS state of registration / the GLEIF address
...
EV = events(); wire_by = {}
for e in EV:
    if e.get('wire') and 'RCS' not in e.get('read_by', '') and 'Perplexity' not in e.get('read_by', ''): wire_by.setdefault(e['exchange'] + '|' + e['ticker'], Counter())[e['wire'].split(' (')[0]] += 1
_src = open(r'C:\ALLOOLOO\CM-KG\RAILS\fr-width0\enrich.py', encoding='utf-8').read()
REG_LIST = eval(re.search(r'^REG_LIST = (\[.*?\])\s*$', _src, re.M | re.S).group(1))
AUD_FIRMS = [('KPMG', r'\bKPMG\b'), ('Deloitte', r'\bDeloitte\b'), ('PricewaterhouseCoopers', r'PricewaterhouseCoopers|\bPwC\b'), ('Ernst & Young', r'Ernst\s*&\s*Young|\bEY\b'), ('Forvis Mazars', r'Mazars'), ('Grant Thornton', r'Grant Thornton'), ('BDO', r'\bBDO\b'), ('RSM', r'\bRSM\b'), ('Crowe', r'\bCrowe\b'), ('Exco', r'\bExco\b'), ('Baker Tilly', r'Baker Tilly'), ('In Extenso', r'In Extenso'), ('Deloitte & Associés', r'Deloitte & Associ'), ('Cabinet', r'\bCabinet\b')]
def firm_key(s):
...
def confirm_issuer(r):
    ...
    if (row.get('LEI State') or '') == 'conflict':
        ...
        acn = row.get('SIREN') or ''; ras = re.sub(r'[^A-Z0-9]', '', (rec.get('registeredAs') or '').upper())
        if rec.get('registeredAt') == ('RA000189', 'RA000192') and acn and ras == acn and 'name-exact' in (row.get('Register read by') or '') + (row.get('LEI read by') or ''): out['fields']['LEI'] = {'state': 'confirmed', 'second': [RCS.get(acn, {}).get('src', '')], 'read_by2': 'SIREN on the LEI record equals the name-exact register row'}
    ...
    v = row.get('SIREN') or ''; ca = claude_acn.get(k, {})
    if v:
        ras = re.sub(r'[^A-Z0-9]', '', (rec.get('registeredAs') or '').upper())
        if 'LEI record' in (row.get('Register read by') or ''):
            out['fields']['SIREN'] = {'state': 'confirmed', 'second': [ca.get('source', '')], 'read_by2': 'RCS / RNE register (State company search) candidates adjudicated by Claude return the same SIREN'} if (ca.get('ruling') or '') == v else {'state': 'sourced', 'second': [], 'read_by2': ''}
        elif ras == v: out['fields']['SIREN'] = {'state': 'confirmed', 'second': [rec.get('src', '')], 'read_by2': 'GLEIF LEI record registeredAs equals the name-matched SIREN'}
        else: out['fields']['SIREN'] = {'state': 'sourced', 'second': [], 'read_by2': ''}
    elif ca.get('ruling'): out['fields']['SIREN'] = {'state': 'filled', 'value': ca['ruling'], 'second': [ca.get('source', '')], 'read_by2': 'adjudicated by Claude: ' + (ca.get('reason') or '')[:120]}
    # registry
    ...
    v = row.get('State') or ''
    if v:
        g_reg = (rec.get('legal_region') or ''); a = RCS.get(row.get('SIREN') or '', {})
        alt = (g_reg[3:] if g_reg.startswith('AU-') else '') if 'RCS' in (row.get('State read by') or '') or 'contact' in (row.get('State read by') or '') else (a.get('state') or '')
        if alt:
