r"""Rebuilds CM-KG\DISCLOSURE\fr-disclosure.xlsx and events\fr-events.jsonl with passes 2 and 3: rematch events, Gemini AGM / record-date findings,
...
fr-disclosure-width1.xlsx (+ fr-events-width1.jsonl); a versioned copy lands in the pond."""
...
X = r'C:\ALLOOLOO\CM-KG\DISCLOSURE\fr-disclosure.xlsx'; J = r'C:\ALLOOLOO\CM-KG\DISCLOSURE\events\fr-events.jsonl'
BX = X.replace('fr-disclosure.xlsx', 'fr-disclosure-width1.xlsx'); BJ = J.replace('fr-events.jsonl', 'fr-events-width1.jsonl')
...
mt = wb['Method']; mt.append([]); mt.append(['Passes 2 and 3 (ORDER-016 Part B, ' + TODAY.isoformat() + ')', 'Engine duty and count on the France Disclosure workbook'])
...
wb.save(X); _ad = pond.assembled(NODE); shutil.copy(X, os.path.join(_ad, 'fr-disclosure.xlsx')); shutil.copy(J, os.path.join(_ad, 'fr-events.jsonl'))
