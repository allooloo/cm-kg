...
by_iss = {}
for e in EV:
    if 'RCS' in e.get('read_by', ''): continue
...
def fetch(r):
    ...
    prompt = (f"Issuer: {r['name']} ({r['exchange']}: {r['ticker']}). Below are this issuer's AMF BDIF regulated information over the last 12 months (date | headline | link) and excerpts of its lodged documents.\n"
