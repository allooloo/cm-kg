"""Shared pieces for the France Fill + Confirm rail (passes 2 and 3). Same shape as the UK rail; France pockets. Pond-native:
raw\\ is a junction to the open drop POND\\fr-cm-kg\\fill-confirm\\<date>\\; jload reads every drop (newest wins per key)."""
import os, sys, re, json, time, threading, html, datetime
import requests, openpyxl
sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\fr-width1'); sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\fr-width0')
import keys
import pond
...
NODE = 'fr-cm-kg'
ISSUERS_XLSX = pond.latest_assembled(NODE, 'fr-issuers.xlsx') or r'C:\ALLOOLOO\CM-KG\ISSUERS\fr-issuers.xlsx'
EVENTS_JSONL = pond.latest_assembled(NODE, 'fr-events.jsonl') or r'C:\ALLOOLOO\CM-KG\DISCLOSURE\events\fr-events.jsonl'
...
def sgx_docs(page_url, want=None):
    """an EQS News announcement page on regulator lists its attachments (regulator/1.0.0/corporate-announcements/<ID>/<file>.pdf); returns [(pdf_url, name)] (filtered by the 'want' regex when given)"""
    ...
    for m in re.finditer(r'<a[^>]+href="([^"]+)"[^>]*>(.*?)</a>', x.text, re.S):
        ...
        u = h if h.startswith('http') else 'https://regulator' + h
