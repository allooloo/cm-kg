...
finds that no engine sourced (an auditor, a registrar, a meeting or period date in a French text) is a FLAG for the Review tab,
never a record value. Label: 'review by Mistral (fr)'. Writes raw/mistral_reads.jsonl (one row per reviewed URL)."""
from fc_common import *
LANG = 'fr'
LABEL = f'review by Mistral ({LANG})'
NATIVE = re.compile(r"(?i)\b(résultats|rapport annuel|assemblée générale|chiffre d'affaires|dividende|nomination|communiqué|franchissement|déclaration)\b")
def native(e): return (e.get('language') or '') in ('fr',) or bool(NATIVE.search(e.get('title', '')))
targets = []
for e in events():
