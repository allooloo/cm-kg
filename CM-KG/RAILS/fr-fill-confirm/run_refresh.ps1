# CM-KG rail — Switzerland passes 2 (Fill) and 3 (Confirm), run after the Width 1 weekly refresh (see CM-KG\RAILS\ch-weekly.ps1). Pond-native; skips on the node lock.
$ErrorActionPreference = 'Continue'
Set-Location $PSScriptRoot
if (Test-Path 'C:\ALLOOLOO\CM-KG\POND\ch-cm-kg\.lock') { 'ch-cm-kg locked (build order in flight): refresh skipped'; exit 0 }
python ..\pond_open.py ch-cm-kg ch-fill-confirm fill-confirm
...
Step 'Width 1 assemble with the live layer' { Push-Location ..\ch-width1; $env:WINDOW_DAYS = '7'; python assemble_disclosure.py; Pop-Location }
Step 'aliases' { $env:THREADS = '5'; python alias_sources.py }
Step 'annual reports + results announcements (SGXNet PDFs)' { $env:THREADS = '4'; python annual_reports.py }
