...
print('ISINs to look up', len(want), 'FR-prefixed', sum(1 for s in want if s.startswith('FR')), flush=True)
...
for lei, isin in rd:
    total += 1
    if isin.startswith('FR'): au += 1
    if isin in want:
        if isin in hits and hits[isin] != lei: hits[isin] = hits[isin] + '|' + lei
        else: hits[isin] = lei
json.dump(hits, open('raw/isin_lei_hits.json', 'w'), indent=0)
json.dump({'mapping_rows': total, 'fr_rows_in_file': au, 'wanted': len(want), 'hits': len(hits), 'file': open('raw/isin-lei-latest.txt').read().strip()}, open('raw/isin_lei_meta.json', 'w'))
print('mapping rows', total, 'FR rows in file', au, 'hits', len(hits), 'of', len(want), 'multi-LEI ISINs', sum(1 for v in hits.values() if '|' in v), flush=True)
