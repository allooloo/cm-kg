"""Newswire releases outside the regulatory feed, via Tavily, for every corporate issuer (the wire of habit first, then the other wires):
   (1) topic=news, days=<window>, wire domains, 20 results  -> published_date from Tavily
   (2) general search "<short name>" announces on the wire domains -> URL date (GlobeNewswire, Business Wire) or Tavily published_date
Hits must carry the issuer's name in the title (name rule enforced again in the assembler). RNS copies on londonstockexchange.com / investegate
are left to investegate.py. Read-by: 'Tavily search (wire domains) + published_date' or '+ URL date'."""
import re
from common import *
DOMS = ['prnewswire.com', 'prnewswire.co.uk', 'globenewswire.com', 'businesswire.com', 'accesswire.com', 'newsfilecorp.com']
INDEX = re.compile(r'globenewswire\.com/(organization|search)|prnewswire\.com/news/[^/]+/?$|prnewswire\.co\.uk/news/[^/]+/?$|accesswire\.com/newsroom|newsfilecorp\.com/company/|businesswire\.com/(news/home/\?|portal)')
def clean(u):
    u = u.split('#')[0]; return re.sub(r'^https?://(rss|secure|www2|ml-eu|mb)\.', 'https://www.', u)
def short_name(n):
    s = re.sub(r'\b(PLC|P\.L\.C\.|Plc|plc|Limited|Ltd\.?|Holdings|Group|Company|Co\.?|Inc\.?|Corp\.?|Corporation|S\.?A\.?|N\.?V\.?)\b\.?', '', n, flags=re.I)
...
def fetch(r):
    ...
    for payload, rb in queries:
        ...
        for x in j.get('results', []):
            ...
            if not w or w == 'RNS' or INDEX.search(u) or u in seen: continue
