"""Step 6 — Tavily reads for the fields no registry publishes: share registrar / securities-services agent (closed list of the Dutch agents within 300
characters of "registrar" / "paying agent" / "listing agent") and newswire of habit (majority of up to three releases on the wire domains that carry
the issuer name; GlobeNewswire and Business Wire carry most Dutch releases). Auditor is NOT read here (annual report link only at Width 0).
...
REG_LIST = [('Euroclear Nederland', r'Euroclear Nederland'), ('ABN AMRO (agent)', r'ABN AMRO'), ('ING Bank (agent)', r'ING'), ('IQ EQ', r'IQ ?EQ'), ('Computershare', r'Computershare'), ('Intertrust / CSC', r'Intertrust|CSC'), ('Rabobank (agent)', r'Rabobank'), ('Van Lanschot Kempen (agent)', r'Kempen')]
def do_reg(r):
    j = tavily({'query': f"{r['name']} registrar paying agent listing agent shareholder register", 'max_results': 6, 'include_raw_content': True})
...
WIRES = {'globenewswire.com': 'GlobeNewswire', 'businesswire.com': 'Business Wire', 'prnewswire.com': 'PR Newswire', 'accesswire.com': 'ACCESS Newswire', 'newsfilecorp.com': 'Newsfile', 'eqs-news.com': 'EQS News'}
...
if __name__ == '__main__':
    task = sys.argv[1]
    if task == 'reg': run('reg', do_reg, lambda r: is_corp(r) and r['isin'].startswith('NL'))
    if task == 'wire': run('wire', do_wire, lambda r: is_corp(r) and r['isin'].startswith('NL'))
