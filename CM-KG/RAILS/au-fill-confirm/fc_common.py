"""Shared pieces for the Australia Fill + Confirm rail (passes 2 and 3). Same shape as the UK rail; Australian pockets. Pond-native:
raw\\ is a junction to the open drop POND\\au-cm-kg\\fill-confirm\\<date>\\; jload reads every drop (newest wins per key)."""
import os, sys, re, json, time, threading, html, datetime
import requests, openpyxl
sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\au-width1'); sys.path.insert(0, r'C:\ALLOOLOO\CM-KG\RAILS\au-width0')
import keys
import pond
...
EVENTS_JSONL = pond.latest_assembled(NODE, 'au-events.jsonl') or r'C:\ALLOOLOO\CM-KG\DISCLOSURE\events\au-events.jsonl'
...
def asx_pdf(viewer_url):
    """the ASX viewer (displayAnnouncement.do?display=pdf&idsId=…) answers HTML with the announcement PDF link (announcements.asx.com.au/asxpdf/…); returns (pdf_url, bytes) or (None, None)"""
