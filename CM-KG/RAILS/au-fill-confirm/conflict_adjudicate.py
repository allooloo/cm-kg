"""Pass 3 — Claude adjudicates the conflicts Pass 3 raised (first source vs second source) for share registry, auditor, ISIN, LEI, ACN, state and
...
COL = {'Share registry': ('Share registry', 'Share registry source', 'Share registry read by'), 'Auditor': ('Auditor', 'Annual report (ASX announcement)', 'Annual report read by'), 'ISIN': ('ISIN', 'ISIN source', 'ISIN read by'), 'LEI': ('LEI', 'LEI source', 'LEI read by'), 'ACN': ('ACN', 'ASIC source', 'ASIC read by'), 'State': ('State', 'State source', 'State read by'), 'Newswire of habit': ('Newswire of habit', 'Newswire releases seen', 'Newswire read by')}
...
def rule(it):
    ...
    note = {'Newswire of habit': "Note: reading B is the channel carrying the majority of this issuer's announcements over 12 months; the ASX platform is every ASX issuer's primary channel.\n",
            'Auditor': "Note: reading A comes from the independent auditor's report in the latest annual report lodged on ASX (the most authoritative source); reading B is page text.\n",
            'State': "Note: reading A is the ASIC state of registration or the ASX contact address; reading B is the GLEIF legal-address region.\n"}.get(it['field'], '')
