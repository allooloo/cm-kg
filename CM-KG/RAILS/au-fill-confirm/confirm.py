"""Pass 3 — Confirm. Every filled or sourced identity field gets a second, independent source where one exists:
  ...
  LEI             second = the ACN on the LEI record (registeredAs at ASIC) equals the ASIC row, else the mapping file vs name-exact
  ACN / ABN       second = the GLEIF record's registeredAs equals the ACN (when the ACN came from the name route), or Claude's ruling agrees (when from the LEI route)
  ...
  State           second = GLEIF legal-address region vs the ASIC state of registration / the ASX contact address
...
EV = events(); wire_by = {}
for e in EV:
    if e.get('wire') and 'ASIC' not in e.get('read_by', ''): wire_by.setdefault(e['exchange'] + '|' + e['ticker'], Counter())[e['wire'].split(' (')[0]] += 1
_src = open(r'C:\ALLOOLOO\CM-KG\RAILS\au-width0\enrich.py', encoding='utf-8').read()
REG_LIST = eval(re.search(r'^REG_LIST = (\[.*?\])\s*$', _src, re.M | re.S).group(1))
AUD_FIRMS = [('KPMG', r'\bKPMG\b'), ('Deloitte', r'\bDeloitte\b'), ('PricewaterhouseCoopers', r'PricewaterhouseCoopers|\bPwC\b'), ('Ernst & Young', r'Ernst\s*&\s*Young|\bEY\b'), ('BDO', r'\bBDO\b'), ('Grant Thornton', r'Grant Thornton'), ('RSM', r'\bRSM\b'), ('Pitcher Partners', r'Pitcher Partners'), ('HLB Mann Judd', r'HLB Mann Judd'), ('Hall Chadwick', r'Hall Chadwick'), ('Stantons', r'Stantons'), ('Nexia', r'\bNexia\b'), ('Moore Australia', r'\bMoore\b'), ('William Buck', r'William Buck'), ('Elderton', r'Elderton'), ('Crowe', r'\bCrowe\b'), ('Bentleys', r'Bentleys'), ('PKF', r'\bPKF\b'), ('Rothsay', r'Rothsay'), ('Mazars', r'Mazars'), ('Walker Wayland', r'Walker Wayland'), ('Butler Settineri', r'Butler Settineri'), ('Ashurst', r'Ashurst'), ('Connect Audit', r'Connect Audit'), ('Hardwickes', r'Hardwickes')]
def firm_key(s):
...
def confirm_issuer(r):
    ...
    if (row.get('LEI State') or '') == 'conflict':
        ...
        acn = row.get('ACN') or ''; ras = re.sub(r'\D', '', rec.get('registeredAs') or '')
        if rec.get('registeredAt') == 'RA000014' and acn and ras == acn and 'name-exact' in (row.get('ASIC read by') or '') + (row.get('LEI read by') or ''): out['fields']['LEI'] = {'state': 'confirmed', 'second': [ASIC.get(acn, {}).get('src', '')], 'read_by2': 'ACN on the LEI record equals the name-exact ASIC row'}
    ...
    v = row.get('ACN') or ''; ca = claude_acn.get(k, {})
    if v:
        ras = re.sub(r'\D', '', rec.get('registeredAs') or '')
        if 'LEI record' in (row.get('ASIC read by') or ''):
            out['fields']['ACN'] = {'state': 'confirmed', 'second': [ca.get('source', '')], 'read_by2': 'ASIC bulk candidates adjudicated by Claude return the same ACN'} if (ca.get('ruling') or '') == v else {'state': 'sourced', 'second': [], 'read_by2': ''}
        elif ras == v: out['fields']['ACN'] = {'state': 'confirmed', 'second': [rec.get('src', '')], 'read_by2': 'GLEIF LEI record registeredAs equals the name-matched ACN'}
        else: out['fields']['ACN'] = {'state': 'sourced', 'second': [], 'read_by2': ''}
    elif ca.get('ruling'): out['fields']['ACN'] = {'state': 'filled', 'value': ca['ruling'], 'second': [ca.get('source', '')], 'read_by2': 'adjudicated by Claude: ' + (ca.get('reason') or '')[:120]}
    # registry
    v = row.get('Share registry') or ''; g = gem.get(k, {}); reads = []
    ...
    if v:
        ...
        if not agree and not dis2:
            first_dom = dom(row.get('Share registry source') or ''); found = [(lab, u) for lab, u in second_read(r, 'reg') if dom(u) != first_dom]
        ...
        if agree: out['fields']['Share registry'] = {'state': 'confirmed', 'second': [agree[0][0]], 'read_by2': agree[0][1]}
        elif dis2: out['fields']['Share registry'] = {'state': 'conflict', 'second': dis2[:2], 'read_by2': 'a lab read names a different registry'}
        elif disagree and disagree.most_common(1)[0][1] >= 2: out['fields']['Share registry'] = {'state': 'conflict', 'second': [f'{lab} ({n} pages)' for lab, n in disagree.most_common(2)], 'read_by2': 'Tavily second read names a different registry'}
        else: out['fields']['Share registry'] = {'state': 'sourced', 'second': [], 'read_by2': ''}
    elif reads: out['fields']['Share registry'] = {'state': 'filled', 'value': reads[0][0], 'second': [reads[0][1]], 'read_by2': reads[0][2]}
    ...
    if v and c:
        ...
        if top == 'ASX' and v.startswith('ASX') or norm(top).split()[0] == norm(v).split()[0]: out['fields']['Newswire of habit'] = {'state': 'confirmed', 'second': [f'Width 1 events: {n} of {sum(c.values())} announcements via {top}'], 'read_by2': 'Width 1 12-month event set (independent observation)'}
    ...
    v = row.get('State') or ''
    if v:
        g_reg = (rec.get('legal_region') or ''); a = ASIC.get(row.get('ACN') or '', {})
        alt = (g_reg[3:] if g_reg.startswith('AU-') else '') if 'ASIC' in (row.get('State read by') or '') or 'contact' in (row.get('State read by') or '') else (a.get('state') or '')
        if alt:
