"""Pass 2.4 — Gemini (gemini-flash-latest, long context) reads each issuer's 12-month announcement set (headlines with dates and links; ASX headlines are
the category names, so the read is over the headline set plus any cached Appendix 4E/4D text) for issuers with no agm_record_date event or no share
...
by_iss = {}
for e in EV:
    if 'ASIC' in e.get('read_by', ''): continue
...
def fetch(r):
    ...
    text = '\n'.join(parts)[:300000]; need_reg = not (rows.get(k, {}).get('Share registry') or '')
    prompt = (f"Issuer: {r['name']} ({r['exchange']}: {r['ticker']}). Below are this issuer's ASX announcements over the last 12 months (date | headline | link) and excerpts of its lodged documents.\n"
              ...
              + ("Task 2: name the share registry if an excerpt states it (e.g. 'Computershare Investor Services Pty Limited', 'MUFG Corporate Markets', 'Boardroom Pty Limited', 'Automic'), with the document link.\n" if need_reg else "")
...
if __name__ == '__main__':
    targets = [r for r in issuers if by_iss.get(key(r)) and (key(r) not in have or not (rows.get(key(r), {}).get('Share registry') or ''))]
