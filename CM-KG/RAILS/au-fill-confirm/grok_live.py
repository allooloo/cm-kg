...
reinstatements on ASX and NSX, plus announcements for issuers the sources left silent. Every finding must carry a URL; findings without one are dropped.
...
def halts():
    out = []; usage = []
    for ex, label in [('ASX', 'Australian Securities Exchange (ASX)'), ('NSX', 'National Stock Exchange of Australia (NSX)')]:
        items, res = ask(f"List every trading halt, suspension from quotation and reinstatement to quotation for companies on the {label} announced in the last {HOURS} hours. Use web search and X search. "
                         "Return JSON only: {\"items\": [{\"issuer\": \"legal name\", \"code\": \"ASX code\", \"kind\": \"trading_halt|suspension|reinstatement\", \"date\": \"YYYY-MM-DD\", \"title\": \"headline\", \"url\": \"source URL\"}]}. Only items with a real source URL.")
...
def silent(keys_silent):
    ...
    for k in keys_silent:
        r = byk[k]
        items, res = ask(f"List ASX announcements and press releases issued by {r['name']} (ASX: {r['ticker']}) in the last {HOURS} hours, with the URL of each. Return JSON only: {{\"items\": [{{\"date\": \"YYYY-MM-DD\", \"title\": \"headline\", \"url\": \"URL\"}}]}}. Only real announcements with URLs; empty list if none.")
        usage.append(res.get('usage'))
        for it in items:
            u = str(it.get('url', ''))
