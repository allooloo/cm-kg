"""Step 5b — GLEIF legal-name candidates (fuzzycompletions) for roster issuers whose ISIN is not in the mapping file.
Same rule as Canada: a candidate becomes the LEI only when the GLEIF legal name equals the roster name after case / space / punctuation
normalisation ('GLEIF name-exact'); legal-form expansions and dropped articles are fuzzy and stay in Gaps. Writes raw/lei_match.jsonl."""
...
for mkt, blk in ex['markets'].items():
    for x in blk['rows']:
        if x.get('isin') not in hits: names.setdefault(x['issuername'], 'LSE|' + x['tidm'])
if os.path.exists('raw/aquis.json'):
    for r in json.load(open('raw/aquis.json', encoding='utf-8'))['rows']:
        if r.get('isin') not in hits: names.setdefault(r['name'], 'AQSE|' + r['symbol'])
def name_key(s):
