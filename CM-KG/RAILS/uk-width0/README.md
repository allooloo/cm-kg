...
| 9b | `ch_api.py` | Companies House REST API (key in `AGENT KEYS\companieshouse.txt`, added 2026-09-11; 600 calls per five minutes) | number via API name search for rows still unmatched (exact name, public company: 47 new, 6 ambiguous → Gaps), profile refresh on every matched row (1,177), the registry's own `jurisdiction` field, SIC, latest accounts filing document link (1,164) | `Companies House REST API (company profile)` |
...
| Companies House REST API | HTTP 401 without a key; HTTP 200 with the key registered 2026-09-11 (application allooloo-cm-kg) | step 9b: name search for unmatched rows, profile refresh, jurisdiction field, latest accounts document |
...
## Fill (first run 2026-09-11, after the API pass; pre-API figures in brackets where they moved)
...
| LEI | 95.9 % | 98.0 % | 92.3 % |
| Companies House number / accounts link | 71.7 % (68.5) | 80.7 % (77.5) | 69.2 % |
| SIC code(s) | 69.6 % (68.5) | 78.7 % (77.5) | 69.2 % |
| Registered office | 96.4 % (96.0) | 98.2 % | 100 % |
...
- **Companies House number** missing on 393 rows after the API pass (440 before): funds and overseas issuers whose LEI is registered outside Companies House (Jersey, Guernsey, Bermuda, BVI, Cayman, Ireland …), plus UK-named issuers whose exchange name differs from their registered name (trading names, "Group"/"Holdings" variants) — the API name search finds no exact-name public company for 387 and two or three for 6. Those need an alias channel or a Fill pass, not a looser match.
