...
No scheduled task (global BUILD lock). Japan weekly: `jp-width1\run_refresh.ps1` (EDINET daily lists, 7 days) then the Fill pass parts (`jp-fill-confirm`: document shards → Gemini loop → Perplexity fast → Grok → Mistral review → rebuilds → spend). Korea weekly: `kr-width1\run_refresh.ps1` (DART list per corp code, 7 days) then `kr-fill-confirm` (DART auditor endpoint → document shards → Gemini loop → Perplexity fast → Grok → Mistral review → rebuilds → spend); DART cap 20,000 calls a day. Hong Kong: Width 0 only by order (`hk-width0`, re-run into a new drop on order); no Width 1, no Fill, no door until a local partner. The GLEIF golden copy (`gleif-golden\download.py` + `extract.py`) refreshes once per build day and serves every local LEI join. United States: see the next section (the EDGAR block was the noreply contact address, resolved Sept 12 with the mailbox of record).

## Current setting — United States (set Sept 12 2026)
No scheduled task (global BUILD lock). The rail is the Container Apps job `cmkg-us-rail` in East US (image `cmkg-us-job`, `CM-KG\RAILS\us-job\us_pipeline.py`): EDGAR company_tickers_exchange + submissions per CIK under 8 requests a second with the declared User-Agent "Allooloo Technologies Corp. developers@allooloo.ai", GLEIF join from the staged golden copy in the pond, Width 1 from the filings index, Fill (10-K iXBRL auditor tag, Gemini transfer agent, Perplexity fast pages), door render straight into the East US store (`door/…`; every raw file under `pond/width0/<date>/`). When the lock lifts: weekly = `az containerapp job start -g allooloo-cmkg-eastus -n cmkg-us-rail` on the US market clock (Container Apps cron trigger, its own order). EDGAR refuses a GitHub noreply address in the User-Agent (403 from every network); the mailbox of record is the one that answers.
